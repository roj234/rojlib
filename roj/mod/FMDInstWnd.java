 ...
 * @since 2020/8/10
 */
public class FMDInstWnd extends JFrame {
    static final TaskExecutor handler = new TaskExecutor();

    public FMDInstWnd(boolean auto) {
        ...
        if(!auto) {
            ...
            install.addActionListener((ev) -> {
                if(!handler.sleeping())
                    return;
                handler.execute(FMDInstWnd::install);
            });
            ...
            add(terminal);
            final JLabel label = new JLabel("自动安装中... 如果出错请退出重来");
            MCLauncher.waitOut = new PrintStream(new DummyOutputStream()) {
                @Override
    ...
    private static void quickInst(String code) {
        ...
        String mcVer = MCLauncher.launcher_conf.getString("mc_version");
    ...
    private static List<File> downloadMC(File mcRoot) {
        ...
        MCLauncher.VersionSelectWindow wnd = new MCLauncher.VersionSelectWindow(vList.raw(), false);
        synchronized (wnd) {
            try {
                wnd.wait();
        ...
        wnd = new MCLauncher.VersionSelectWindow(vList.raw(), true);
        synchronized (wnd) {
            try {
                wnd.wait();
            } catch (InterruptedException e) {
    ...
    private static void doInstall(File mcRoot, File mcJson, List<String> prefilledAnswers) {
        try {
            if(ModDevelopment.changeVersion(mcRoot, mcJson, new Gui(prefilledAnswers)) != 0) {
    ...
    public static void main(String[] args) {
        UIUtil.systemLook();
        MAIN_CONFIG.size();
        frame = new FMDInstWnd(args.length > 0);
    ...
    private static final class Gui extends IGuiHelper {
        String msg;
        String[] objs;
        ...
        boolean getBoolean(String msg) {
            return JOptionPane.showConfirmDialog(frame, "是否需要清除该死的ParametersAreNonnullByDefault之类注解", "询问", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION;
        }
        ...
        int getNumberInRange(int min, int max) {
            ...
            String s;
            do {
                s = (String) JOptionPane.showInputDialog(frame, msg != null ? msg : "请选择", "询问", JOptionPane.QUESTION_MESSAGE, null, obj, obj[0]);
            } while (s == null);
        ...
        String userInput(String msg, boolean optional) {
            ...
            if(obj == null) {
                String s;
                do {
                    s = JOptionPane.showInputDialog(frame, this.msg != null ? (this.msg.startsWith("$") ? msg + this.msg.substring(1) : this.msg) : msg, "询问", JOptionPane.QUESTION_MESSAGE);
                ...
                String s;
                do {
                    s = (String) JOptionPane.showInputDialog(frame, this.msg != null ? this.msg : msg, "询问", JOptionPane.QUESTION_MESSAGE, null, obj, obj[0]);
                } while (s == null && !optional);
