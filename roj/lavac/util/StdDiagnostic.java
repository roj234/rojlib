 ...
 */
public final class StdDiagnostic implements DiagnosticListener<ClassContext> {
    int err, warn, op;
    ...
    private static String buildErrorMessage(Diagnostic<? extends ClassContext> diagnostic, EnumMap<Diagnostic.Kind, MutableInt> map) {
    ...
    private static String getErrorMsg(Diagnostic.Kind kind, EnumMap<Diagnostic.Kind, MutableInt> kinds) {
        switch (kind) {
            ...
            case ERROR:
                if(kinds != null)
                    kinds.computeIfAbsent(kind, k2i).incrementAndGet();
                return "错误";
            case OTHER:
                return "其他";
            case WARNING:
                if(kinds != null)
                    kinds.computeIfAbsent(kind, k2i).increment();
                return "警告";
            case MANDATORY_WARNING:
                if(kinds != null)
                    kinds.computeIfAbsent(Diagnostic.Kind.WARNING, k2i).increment();
                return "强警告";
        }
     ...
     */
    @Override
    public void report(Diagnostic<? extends ClassContext> diagnostic) {
