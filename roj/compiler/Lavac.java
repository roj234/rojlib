...
import roj.compiler.diagnostic.StdDiagnostic;
...
public final class Lavac {
	...
	public static void main(String[] args) {
		...
		u.newMethod(Opcodes.ACC_PUBLIC|Opcodes.ACC_STATIC|Opcodes.ACC_FINAL, "test2", "(ID)J");
		u.getImportCls().put("Test", "roj/compiler/api/Test");
		ctx.addCompileUnit(u);

		cache.setClass(u);
		cache.setMethod(u.methods.get(0));

		try {
			String seq = IOUtil.readUTF(new File("D:\\mc\\FMD-1.5.2\\t.java"));
			u.lex().init(seq);
			BlockParser bp = cache.bp.retain(null);
			bp.ctx2 = cache;
			bp.parseMethod(u, u.methods.get(0), Collections.emptyList(), 0, seq.length());
			MethodWriter cw = bp.cw;
			cw.one(Opcodes.RETURN);
			cw.visitSize(30,30);
			if (!ctx.hasError) {
				cw.finish();
				MethodNode mn = u.methods.get(0);
