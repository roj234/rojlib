...
import roj.compiler.util.TypeCast;
...
public class BlockParser {
	...
	public BlockParser(MethodWriter ctx) {
		this.cw = ctx;
		this.depth = -1;
	...
	private void statement(Word w) {
		try {
			...
			switch (w.type()) {
				...
				case left_l_br: block(); except(right_l_br); break;
	...
	private void _return() throws ParseException {
		...
		TypeCast.Cast cast = ctx2.castTo(expr == null ? Type.std(Type.VOID) : expr.type(), rt,-8);
		if (cast.type < 0) {
			// todo
		}
	...
	private void _try() throws ParseException {
		...
		if (w.type() == FINALLY) {
			...
			if (prevHook == null || !ctx.isSpecEnabled(CompilerConfig.FINALLY_OPTIMIZE)) {
				...
				// 副本的 2/3: return劫持
				if (used > 0) {
					boolean isVoid = cw.mn.returnType().type == VOID;
					cw.label(hook);
				...
				Variable return_value = cw.mn.returnType().type == VOID ? null : tmpVar("finally优化|返回值", Type.CLASS);
	...
	private void _if() throws ParseException {
		Label ifFalse = condition(true, right_s_br);
	...
	private Label condition(boolean checkBracket, short end) throws ParseException {
		if (checkBracket) except(left_s_br);

		ExprParser parser = ep;

		Label ifFalse = new Label();

		ExprNode equ = parser.parse(file, (checkBracket ? 16 : 0));
		if (equ == null) {
			_onError(wr.readWord(), "statement.empty.if");
			return null;
		}

		int i = cw.beginJumpOn(false, ifFalse);
		equ.resolve(cw.ctx1).write(cw, false);
		cw.endJumpOn(i);
	...
	private void _doWhile() throws ParseException {
		...
		Label breakTo2 = condition(true, right_s_br);
	...
	private void _while() throws ParseException {
		...
		Label continueTo = cw.label();
		Label breakTo = condition(true, right_s_br);
		if (breakTo == null) return;
		...
		switch (sType.type) {
			case CLASS:
				if (sType.owner.equals("java/lang/String")) {
					...
					if (ctx2.instanceOf(sType.owner, "java/lang/Enum", -1)) {
						kind = 2;
						break;
					} else if (ctx2.instanceOf(sType.owner, "java/lang/CharSequence", 1)) {
						kind = 3;
						break;
			...
			switch (w.type()) {
				case CASE:
					...
					while (true) {
						...
						// todo check twice
						if (!(expr = expr.resolve(null)).isConstant())
	...
	private void define(IType type) throws ParseException {
		if (type == null) {
			...
			type = file.resolveType(CompileUnit.TYPE_PRIMITIVE);
		}
