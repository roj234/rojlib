...
final class DotGet implements VarNode {
	...
	final ExprNode resolveEx(CompileContext ctx, Invoke invoke) throws ResolveException {
		...
		if (state >= 0) {
			...
			fType = parent.type();
			if (fType.isPrimitive()) ctx.report(Diagnostic.Kind.ERROR, "symbol.error.derefPrimitiveField", fType);
			...
			if (error != null) {
				ctx.report(Diagnostic.Kind.ERROR, error);
				return this;
			}
		...
		if (i < length) for (;;) {
			...
			if ((fn.modifier & Opcodes.ACC_STATIC) != 0) {
				ctx.report(Diagnostic.Kind.MANDATORY_WARNING, "symbol.warn.static_on_half", part, fn.name(), "symbol.field");
			}
		...
		if (bits != EMPTY_BITS) {
			state |= FINAL_FIELD;
			if ((state&ARRAY_LENGTH) != 0) ctx.report(Diagnostic.Kind.ERROR, "dotGet.error.illegalArrayLength");
