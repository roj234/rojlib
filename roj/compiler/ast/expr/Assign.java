...
final class Assign implements ExprNode {
	...
	public ExprNode resolve(CompileContext ctx) {
		...
		else ctx.report(Diagnostic.Kind.ERROR, "assign.error.final", left);
		...
		if (prev.getClass() == Cast.class) {
			...
			if ((castType == -1 || castType == -2) && !(left instanceof LocalVariable))
				ctx.report(Diagnostic.Kind.NOTE, "assign.incompatible.redundant_cast");
		}
	...
	public void write(MethodWriter cw, boolean noRet) {
		...
		if (right instanceof Binary br) {
			if (br.left.equalTo(left)) {
				if (sameVarShrink(cw, br, noRet, br.right)) return;
			} else if (br.left.isConstant() && br.right.equalTo(left)) {
	...
	public boolean equalTo(Object o) {
		...
		return assign.left.equalTo(left) && assign.right.equalTo(right);
	}
}