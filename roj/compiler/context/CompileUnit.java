...
public final class CompileUnit extends ConstantData {
	...
	public boolean S0_Init() throws IOException, ParseException {
		...
		wr.retractWord();
		wr.clearRetract(); // TODO This method changed
	...
	private void body() throws ParseException {
		...
		while (wr.hasNext()) {
			...
			switch (w.type()) {
				...
				case right_l_br: break mfsdcLoop;
			...
			switch (w.type()) {
				case left_l_br: // static initializator
			...
			if (w.type() == left_s_br) { // method
				...
				if ((acc & (Opcodes.ACC_ABSTRACT|Opcodes.ACC_NATIVE)) != 0 ||
					...
					if (w.type() != left_l_br) throw wr.err("必须包含方法体");
	...
	private IType _genericUse(CharSequence type, int flag) throws ParseException {
		...
		if (type != null) {
			...
			if ((flag & GENERIC_INNER) != 0) {
				Type type1 = (Type) _type(wr, IOUtil.getSharedCharBuf(), TYPE_PRIMITIVE|TYPE_OPTIONAL);
				if (type1 == null) {
					wr.retractWord();
					// 钻石操作符<>
	...
	private static int skipCode(JavaLexer wr, int type) throws ParseException {
		...
		while (wr.hasNext()) {
			...
			switch (w.type()) {
				case lss: G++; break;
				case left_l_br: L++; break;
				case left_m_br: M++; break;
				case left_s_br: S++; break;
				case right_l_br: L--;
					if (L < 0) {
	...
	public void S2_Parse() throws ParseException {
		...
		// 禁止泛型异常
		if (signature != null && ctx1.instanceOf("java/lang/Throwable", parent, -1)) {
			...
			if (list != null) {
				...
				for (int j = 0; j < classes.size(); j++) {
					...
					if (info == null) {
						...
						if (!ctx1.instanceOf("java/lang/Throwable", info.name(), -1)) {
	...
	private boolean _accessible(IClass target, char acc) {
		...
		switch (acc & (Opcodes.ACC_PUBLIC|Opcodes.ACC_PROTECTED|Opcodes.ACC_PRIVATE)) {
			...
			case Opcodes.ACC_PROTECTED: if (ctx1.instanceOf(name, target.name(), 0)) return true;
	...
	public IType resolveType(@MagicConstant(flags = {TYPE_PRIMITIVE,TYPE_OPTIONAL,TYPE_AUTO,TYPE_STAR,TYPE_GENERIC,TYPE_LEVEL2}) int flag) throws ParseException {
