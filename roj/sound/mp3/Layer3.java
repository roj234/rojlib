...
final class Layer3 extends Layer {
	...
	static final TaskPool DECODER = TaskPool.Common();
	...
	public Layer3(Header header, AudioBuffer audio) {
		...
		//---------------------------------------------------------------------
		int sfreq = this.header.getSamplingRateIndex();
		...
		switch (sfreq) {
			...
			case 8:
				...
				break;
			default:
				throw new IllegalStateException("Unexpected value: " + sfreq);
		}
		...
		if (isMPEG1) {
			...
			for (int gr = 0; gr < 2; gr++) {
				for (int ch = 0; ch < channels; ch++) {
					...
					if ((info.window_switching_flag) != 0) {
						...
						if (info.block_type == 0) {return -1;} else if (info.block_type == 2 && info.mixed_block_flag == 0) {info.r0_len = 8;} else info.r0_len = 7;
			...
			for (int ch = 0; ch < channels; ch++) {
				...
				if ((info.window_switching_flag) != 0) {
					...
					info.subblock_gain[1] = bs.get2(3);
					info.subblock_gain[2] = bs.get2(3);
					if (info.block_type == 0) {return -1;} else if (info.block_type == 2 && info.mixed_block_flag == 0) {info.r0_len = 8;} else {
						info.r0_len = 7;
						info.r1_len = 20 - info.r0_len;
					}
				} else {
	...
	static {
		...
		nr_of_sfb = new byte[][][] {
			...
			{{6, 5, 5, 5}, {6, 5, 7, 3}, {11, 10, 0, 0}, {7, 7, 7, 0}, {6, 6, 6, 3}, {8, 8, 5, 0}}, {{9, 9, 9, 9}, {9, 9, 12, 6}, {18, 18, 0, 0}, {12, 12, 12, 0}, {12, 9, 9, 6}, {15, 12, 9, 0}},
	...
	private void getScaleFactors_2(int gr, int ch) {
		...
		int slen;
		if ((ch > 0) && header.isIntensityStereo()) {slen = i_slen2[ci.scalefac_compress >> 1];} else slen = n_slen2[ci.scalefac_compress];
	...
	// MPEG-1
	private static final byte[] sfLen = {0 << 4 | 0, 1 << 4 | 0, 2 << 4 | 0, 3 << 4 | 0, 0 << 4 | 3, 1 << 4 | 1, 2 << 4 | 1, 3 << 4 | 1, 1 << 4 | 2, 2 << 4 | 2, 3 << 4 | 2, 1 << 4 | 3, 2 << 4 | 3,
										 3 << 4 | 3, 2 << 4 | 4, 3 << 4 | 4};
