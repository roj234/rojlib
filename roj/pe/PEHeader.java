...
public class PEHeader implements Writable {
    ...
    public void toByteArray(PEFile owner, ByteList w) {
        ...
        if (format == FORMAT_PE32) {
            w.putIntLE((int) stackReservedSize)
             .putIntLE((int) stackCommitSize)
             .putIntLE((int) heapReservedSize)
             .putIntLE((int) heapCommitSize);
        } else {
            w.putLongLE(stackReservedSize)
             .putLongLE(stackCommitSize)
             .putLongLE(heapReservedSize)
             .putLongLE(heapCommitSize);
        }
    ...
    public String toString() {
        ...
        sb.append("PE公共头部")
          .append("\n  目标CPU:     ").append(CPUType.toString(cpuType))
          .append("\n  区段:        ").append((int)sectionCount)
          .append("\n  时间戳:      ").append(new ACalendar().formatDate("Y-m-d H:i:s", getTimestamp()))
          .append("\n  符号表:      0x").append(Integer.toHexString(symbolTableOffset)).append(" @ Size=").append(symbolCount & 0xFFFFFFFFL)
          .append("\n  可选头大小:  ").append((int)optHeaderSize)
          .append("\n  标志:        ").append(Integer.toBinaryString(characteristics))
          .append("\nPE可选头部");
        
        sb.append("\n  类型:        0x").append(Integer.toHexString(format)).append(" (").append(formatToString(format)).append(")")
          .append("\n  链接器版本:  ").append(linkerVersion >> 8).append('.').append(0xFF & linkerVersion)
          .append("\n  代码大小:    ").append(codeSize)
          .append("\n  定义数据大小:").append(initializedDataSize)
          .append("\n  空白数据大小:").append(uninitializedDataSize)
          .append("\n  入口点:      0x").append(Integer.toHexString(entryPoint))
          .append("\n  代码基址:    0x").append(Integer.toHexString(codeBase));
        if (format == FORMAT_PE32) {
          sb.append("\n  数据基址:    0x").append(Integer.toHexString(dataBase));
        }
        sb.append("\n  映像基址:    0x").append(Long.toHexString(imageBase))
          .append("\n  区段对齐:    ").append(sectionAlign & 0xFFFFFFFFL)
          .append("\n  文件对齐:    ").append(fileAlign & 0xFFFFFFFFL)
          .append("\n  OS版本:      ").append(OSVersion >>> 16).append('.').append(OSVersion & 0xFFFF)
          .append("\n  映像版本:    ").append(imageVersion >>> 16).append('.').append(imageVersion & 0xFFFF)
          .append("\n  子系统版本:  ").append(subsystemVersion >>> 16).append('.').append(subsystemVersion & 0xFFFF)
          .append("\n  映像大小:    ").append(imageSize & 0xFFFFFFFFL)
          .append("\n  头部大小:    ").append(headerSize & 0xFFFFFFFFL)
          .append("\n  校验码:      0x").append(Integer.toHexString(checksum))
          .append("\n  子系统:      ").append(Subsystem.toString(subsystem))
          .append("\n  DLL标志:     ").append(Integer.toBinaryString(DLLCharacteristic))
          .append("\n  栈保留大小:  ").append(stackReservedSize)
          .append("\n  栈提交大小:  ").append(stackCommitSize)
          .append("\n  堆保留大小:  ").append(heapReservedSize)
          .append("\n  堆提交大小:  ").append(heapCommitSize)
          .append("\n  附加表数量:  ").append(dataIndexSize).append(" @ Size=").append(dataIndexSize << 3);
        return sb.toString();
    }
