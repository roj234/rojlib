...
public class UDPasTCP implements ChannelHandler {
	...
	public void channelRead(ChannelCtx ctx, Object msg) throws IOException {
		...
		if (embed == null) {
			pkt.buf = IOUtil.getSharedByteBuf().putShort(0).put((byte) RST);
	...
	public static final class Ctx extends MyChannel {
		...
		public boolean connect0(InetSocketAddress address) throws IOException {
			...
			switch (prot_state) {
				case SEND:
					...
					protocolSend(IOUtil.getSharedByteBuf().put((byte) REQ));
		...
		protected void write(Object o) throws IOException {
			...
			if (!data.isDirect()) {
				data = bp.buffer(true, data.readableBytes()).put(data);
			}
		...
		public void tick(int elapsed) throws IOException {
			...
			if (prot_state < CLOSE && timer != 0 && System.currentTimeMillis() - timer >= half_acceptable_latency) {
				protocolSend(IOUtil.getSharedByteBuf().put((byte) RET).putShort(rcv_seq));
				timer = 0;
			}
		...
		protected void closeHandler() throws IOException {
			if (prot_state == OPEN) {
				protocolSend(IOUtil.getSharedByteBuf().put((byte) RST));
				state = CLOSED;
			}
			...
			for (int i = rcv_seqs.size() - 1; i >= 0; i--) {
				...
				if (b!=null)p.reserve(b);
			}
			rcv_seqs.clear();
			for (int i = snd_seqs.size() - 1; i >= 0; i--) {
				p.reserve(snd_seqs.get(i));
			}
			snd_seqs.clear();
		...
		public void closeGracefully() throws IOException {
			if (state < LOCAL_CLOSE) {
				...
				protocolSend(IOUtil.getSharedByteBuf().put((byte) SHUT));
				state = LOCAL_CLOSE;
			}
		}
		...
		void protocolRead(DynByteBuf buf) throws IOException {
			...
			if (seq == rcv_seq || buf.getU(buf.rIndex) == RST) {
				...
				// send ack
				if ((flag & SEND_ACK) != 0) {
					protocolSend(IOUtil.getSharedByteBuf().put((byte) (ACK|(OPEN<<4))).putShort(rcv_seq));
					flag &= ~SEND_ACK;
				}
				...
				rcv_seqs.i_setSize(Math.max(seq,rcv_seqs.size()));

				rcv_seqs.set(seq-1, alloc().buffer(false, buf.readableBytes()).put(buf));
			}
		}
		...
		private void protocolRead1(DynByteBuf buf) throws IOException {
			...
			switch (k) {
				case ACK|(WAIT<<4): // connection establish
					...
					protocolSend(IOUtil.getSharedByteBuf().put((byte) (ACK|(WAIT<<4))).putInt(rcv_win));
				...
				case DTA: // data
					...
					if (rb.writableBytes() < buf.readableBytes()) {
						...
						DynByteBuf newBuf = p.buffer(false, rb.capacity()<<1);
						newBuf.put(rb);
						p.reserve(rb);
						rb = newBuf;
					}
				...
				case SHUT: // remote close
					if (prot_state == OPEN) {
						protocolSend(IOUtil.getSharedByteBuf().put((byte) (SHUT|(ACK<<4))));
		...
		private boolean onAck(int seq) throws IOException {
			...
			if (seq >= 32767) { // reset seq num
				protocolSend(IOUtil.getSharedByteBuf().put((byte) (ACK|(WIN<<4))));
				snd_seq = 0;
			}
		...
		private void protocolSend(DynByteBuf buf) throws IOException {
			...
			DynByteBuf bb = alloc().buffer(true, buf.readableBytes()+2)
		...
		private int writeData(DynByteBuf buf) throws IOException {
			...
			if (outBuf.writableBytes() < buf.readableBytes() && outBuf.capacity() < outBufMax) {
				...
				DynByteBuf newBuf = p.buffer(true, Math.min(MathUtils.getMin2PowerOf(outBuf.capacity()+buf.readableBytes()), outBufMax));
				newBuf.put(outBuf);
				p.reserve(outBuf);
				outBuf = newBuf;
			}
		...
		private void sendData() throws IOException {
			...
			BufferPool p = alloc();
			DynByteBuf tmp = p.buffer(true, mtu+5);
			try {
				pkt.buf = tmp;
				...
				do {
					...
					if ((flag & SEND_ACK) != 0) {
						flag ^= SEND_ACK;
						tmp.put((byte) (DTA|(RET<<4))).putShort(rcv_seq);
					} else {
						tmp.put((byte) DTA);
					}
					...
					snd_seqs.add(p.buffer(true, t).put(tmp));
					tmp.rIndex -= t;
