...
public class Octree<V extends OctreeEntry> implements MapLike<Node>, Iterable<Node> {
    ...
    public void fill(int level) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            fill(getOrCreateEntry(0b1000 | i),
                 child_x, child_y, child_z,
                 child_unit, level);
        }
    }

    private void fill(Node n,
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (n != null) {
                ...
                if (overlaps(pos, rs, rMax, unit)) {
                    if (origin)
                        rcGetOrigin(n, rMin, rMax, rs, unit >>> 1);
                    else if (!rs.nearest)
                        rcGetAny(n, pos, rMin, rMax, rs, unit >>> 1);
                    else
    ...
    public final void rcGetAny(Node n, Vec3i pos, float rMin, float rMax, LocationSet<V> rs, int unit) {
        ...
        if (n.v.getClass() == ArrayList.class) {
            ...
            for (int i = 0; i < list.size(); i++) {
                int d2 = list.get(i).getPos().distanceSq(pos);
                if (L2 <= d2 && d2 <= H2) {
        ...
        V v = (V) n.v;
        int d2 = v.getPos().distanceSq(pos);
        if (L2 <= d2 && d2 <= H2) {
        ...
        for (int i = 0; i < 8; i++) {
            if ((mask & shl) != 0) {
                ...
                // point in box
                if (overlaps(pos, rs, rMax, unit))
                    rcGetAny(getEntry(k | i), pos, rMin, rMax, rs, unit >>> 1);
            }
            shl <<= 1;
        }
    }
    ...
    public final void rcGetOrigin(Node n, float rMin, float rMax, LocationSet<V> rs, int unit) {
        ...
        for (int i = 0; i < 8; i++) {
            if ((mask & shl) != 0) {
                ...
                // point in box
                if (overlaps(ORIGIN, rs, rMax, unit))
                    rcGetOrigin(getEntry(k | i), rMin, rMax, rs, unit >>> 1);
            }
            shl <<= 1;
        }
    }
    ...
    public final float rcGetNearest(Node n, Vec3i pos, float rMin, float rMax, LocationSet<V> rs, int unit) {
        ...
        for (int i = 0; i < 8; i++) {
            if ((mask & shl) != 0) {
                ...
                // point in box
                if (overlaps(pos, rs, rMax, unit))
                    rMax = rcGetNearest(getEntry(k | i), pos, rMin, rMax, rs, unit >>> 1);
            }
            shl <<= 1;
        }
