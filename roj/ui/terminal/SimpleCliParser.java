...
public class SimpleCliParser {
	...
	private static void add(String s, List<Word> words, int i) throws ParseException {
		if (s.startsWith("\"")) {
			words.add(new Word().init(Word.STRING, i, ITokenizer.removeSlashes(s)));
		} else {
			int number = TextUtil.isNumber(s);
			if (number == 0) {
				words.add(new Word_L(i, Long.parseLong(s), s));
			} else if (number == 1) {
				words.add(new Word_D(Word.DOUBLE, i, Double.parseDouble(s), s));
			} else {
