...
import lac.server.note.ServerClassInject;
import lac.server.util.SharedField;
 ...
 */
@ServerClassInject(dest = "net.minecraftforge.fml.common.network.handshake.FMLHandshakeServerState", child = {"1", "2", "3", "6"}, childTo = {"1", "2", "3", "6"})
public enum NX_HS_S implements IHandshakeState<NX_HS_S> {
    START {
        public void accept(ChannelHandlerContext a, FMLHandshakeMessage b, Consumer<? super NX_HS_S> c) {}
    },
    HELLO {
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg, Consumer<? super NX_HS_S> cons) {
            if (msg instanceof FMLHandshakeMessage.ClientHello) {
                ...
                dsp.setModList(client.modList());
                boolean pass = ModInfo.passHandshake(client.modList());
                ...
                if(!pass) {
                    switch (Config.noPass1Type) {
                        ...
                        case 1: // send rejection
                            cons.accept(ERROR);
                            dsp.rejectHandshake(ModInfo.makeFakeRejections(ctx.channel().remoteAddress(), client.modList()));
                            break;
                ...
                String rejection = ModInfo.checkModList(client.modList(), Side.CLIENT);
                if (rejection != null) {
                    cons.accept(ERROR);
    ...
    WAITINGCACK {
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg, Consumer<? super NX_HS_S> cons) {
            ...
            if (!ctx.channel().attr(NetworkDispatcher.IS_LOCAL).get()) {
                Map<ResourceLocation, ForgeRegistry.Snapshot> snapshot = SharedField.getSnapshot();
    ...
    LOGIN_INIT {
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg, Consumer<? super NX_HS_S> cons) {
            ...
            conn.sendPacket(new SPacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer(32))).writeString("Login Screen")));
