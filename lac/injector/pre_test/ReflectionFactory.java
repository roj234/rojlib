...
public class ReflectionFactory {
    ...
    private final Set<String> invokedThings;

    private ReflectionFactory() {
        invokedThings = Collections.newSetFromMap(new ConcurrentHashMap<>());
        Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
            @Override
            public void run() {
                if (!invokedThings.isEmpty()) {
                    try (FileOutputStream fos = new FileOutputStream("$$INVOKED_THINGS.log")) {
                        for (String s : invokedThings) {
    ...
    public FieldAccessor newFieldAccessor(Field field, boolean override) {
        checkInitted();
        invokedThings.add(field.getClass().getName() + "|" + field.getName() + "|" + field.getType().getName());
    ...
    public MethodAccessor newMethodAccessor(Method method) {
        ...
        StringBuilder sb = new StringBuilder().append(method.getClass().getName()).append("|").append(method.getName()).append("|");
        Class<?>[] x = method.getParameterTypes();
        for (Class<?> cl : x) sb.append(cl.getName());
        invokedThings.add(sb.toString());
    ...
    public ConstructorAccessor newConstructorAccessor(Constructor<?> c) {
        ...
        StringBuilder sb = new StringBuilder().append(c.getClass().getName()).append("||");
        Class<?>[] x = c.getParameterTypes();
        for (Class<?> cl : x) sb.append(cl.getName());
        invokedThings.add(sb.toString());
