...
public final class FMD {
	...
	public static final File
			...
			BIN_PATH = IOUtil.getJar(FMD.class).getParentFile(),
	...
	private static final SerializerFactory CONFIG = SerializerFactory.getInstance()
			.serializeCharsetToString()
			.add(ArtifactVersion.class, MySerializer.class)
			.add(File.class, MySerializer.class);
	...
	private static void registerCommands() {
		...
		c.register(literal("project")
			...
			.then(literal("export").then(argument("项目名称", Argument.oneOf(projects)).executes(ctx -> {
				...
				arc.outputFilename = "mcmake-project-"+proj.getSafeName()+"-"+DateTime.local().format("Ymd", System.currentTimeMillis())+".7z";
		...
		c.register(literal("workspace")
			...
			.then(literal("export").then(argument("工作空间名称", Argument.oneOf(workspaces)).executes(ctx -> {
				Workspace space = CONFIG.serializer(Workspace.class).deepcopy(ctx.argument("工作空间名称", Workspace.class));
				...
				try (var qzfw = new QZFileWriter("workspace-"+space.id+".7z");
					...
					ConfigMaster.YAML.writeObject(CONFIG.serializer(Workspace.class), space, IOUtil.getSharedByteBuf()).writeToStream(qzfw);
			...
			.then(literal("import").then(argument("工作空间归档", Argument.file()).executes(ctx -> {
				...
				try (var archive = new QZArchive(archiveFile);
					...
					Workspace workspace = ConfigMaster.YAML.readObject(CONFIG.serializer(Workspace.class), in);
		...
		c.register(literal("runscript").comment("执行脚本").then(argument("脚本名称", Argument.fileIn(new File(CONF_PATH, "scripts"))).executes(ctx -> {
			...
			try {
				...
				var cd = new ClassDefiner(FMD.class.getClassLoader(), "fmdScript");
				for (CompileUnit file : files) ClassDefiner.defineClass(cd, file);
	...
	private static boolean loadConfig() {
		try {
			config = new YAMLParser().parse(new File(CONF_PATH, "config.yml"), Flags.NO_DUPLICATE_KEY|Flags.ORDERED_MAP).asMap();
		} catch (Exception e) {
	...
	private static void loadEnv() throws IOException, ParseException {
		...
		for (var value : projects.values()) {
			value.init();
			Workspace.registerModule(value, false);
		}
		updatePrompt();
	}
	...
	private static boolean compile(Set<String> args, Project p) throws IOException {
		...
		var context = new ProcessEnvironment(p);
	...
	private static boolean build(Set<String> args, Project p, File jarFile) throws IOException {
		...
		Future<Integer> resources = EXECUTOR.submit(p.getResourceTask(increment == INC_UPDATE ? prevCompile : -1));
		context.initCache();
	...
	private static boolean compile(Set<String> args, Project p, long modifyAfter, ProcessEnvironment context) throws IOException {
