...
final class OjngMapping extends Mapping {
	...
	public void readMojangMap(String map, LineReader slr, List<String> tmp) throws IOException {
		...
		Top1<String> list = new Top1<>((old, latest) -> {
			if (old != null) found[0] = true;
			return latest;
		});
		...
		while (true) {
			...
			if (c == ' ') {
				...
				found[0] = false;
				list.best = null;
				Top1<String> arr1 = (Top1<String>) TextUtil.split(list, line.trim(), ':');
				String val = arr1.best;

				if (!found[0]) {
					if (TextUtil.lastIndexOf(val, ')') != -1) {
						// SBMJ, no line number
						found[0] = true;
					} else {
						// field type: arr[0]
		...
		for (Iterator<Map.Entry<MemberDescriptor, String>> itr = methodMap.entrySet().iterator(); itr.hasNext(); ) {
			...
			String param = U.mapMethodParam(classMap.inverse(), k.rawDesc);
			if (param != k.rawDesc) {
				itr.remove();
