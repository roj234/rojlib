...
**Version: 3.8** | **By: Roj234** | **License: BSD**  
...
-   **JAR 签名**：基于 RojLib 的快速实现，无需 `jarsigner` 依赖（暂不支持时间戳签名）。
...
-   这将生成以下项目结构：
    ...
    └── lib/           # 本地依赖 JAR (不推荐使用，建议通过 dependency 统一管理)
    ```
    ...
    projects:
      - name: mymod
        ...
        dependency:
          ...
          file:../other-mod.jar: BUNDLED  # 本地文件
    ```
...
-   **不与 Gradle 对齐**：MCMake 完全从零开发，因此不默认支持 Kotlin、Architectury 或其他高级管道。
...
2.  移除本地文件依赖，例如切换为带有 SHA1 校验的 Maven 依赖，确保其他人也能获取相同的依赖项。
3.  前文提到不建议使用 `lib` 目录中的本地依赖，因为这些文件并不属于“代码”，理论上也不应该被 VCS 管理。
