...
public class Worley implements Noiser {
	...
	private final LRUCache<LongValue, Vec3d> nTmp = new LRUCache<>(512);
	private final LongValue t = new LongValue();
		...
		for (int i = 0; i < 3; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				if (v == null) {
					...
					v.z = r.noise(x0, y0);
					nTmp.put(new LongValue(t.value), v);
				}
