...
public class DebugRenderer {
    private static SimpleList<AxisAlignedBB> nextDraw = new SimpleList<>();
    private static SimpleList<BlockPos[]> nextLine = new SimpleList<>();
    ...
    public static void region(BlockPos pos) {
        nextDraw.add(new AxisAlignedBB(pos, pos.add(1,1,1)));
    }
    ...
    public static void drawPending() {
        ...
        for (int i = 0; i < nextDraw.size(); i++) {
            ...
            if (EntityHelper.canPlayerSee(box)) {
                ...
                // noinspection all
                ArenaRenderer.INSTANCE.render(new Vec3d(box.minX, box.minY, box.minZ),
        ...
        GL11.glEnd();
        if (nextDraw.size() > 30) nextDraw.clear();
        if (nextLine.size() > 30) nextLine.clear();
