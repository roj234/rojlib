...
public class MockingUtil {
    ...
    public static void registerAdapter() {
        registeredAdapters.put(getVersionNumberFromMCString("1.16.5"), RuleBasedPacketAdapter::new);
    }
    ...
    public static int getVersionNumberFromMCString(String version) {
        if (versionNumbers == null) {
            ...
            try {
                ...
                for (String s : new SimpleLineReader(IOUtil.readUTF("META-INF/version.map"), true)) {
    ...
    public static List<String[]> mockModsFromServerReject(ITextComponent message) {
        String msg = message.getFormattedText();
        if (message instanceof TextComponentTranslation && (msg.contains("utdate") || msg.contains("过期"))) {
            String ver = ((TextComponentTranslation) message).getFormatArgs()[0].toString();
            System.out.println("服务器版本 " + ver);
            if (autoMockVersion) {
                int v = getVersionNumberFromMCString(ver);
                if (v > 0) mockProtocol = v;
            }
            return null;
        }
        if (!msg.startsWith("Server Mod rejections:\n")) return null;
        SimpleList<String[]> tmp = new SimpleList<>();
        for (String key : new SimpleLineReader(msg.substring(23))) {
            int pos = key.indexOf(':');
            if (pos < 0) return null;
            if (!key.startsWith(" Requires version ", pos + 1)) return null;
            int from = pos + 19;

            int nextSpace = key.indexOf(' ', from);
            if (key.startsWith("range", nextSpace)) {
                from = nextSpace;
                nextSpace = key.indexOf(' ', nextSpace);
            }

            try {
                tmp.add(new String[]{
                    key.substring(0, pos),
                    key.substring(from, nextSpace)
                });
    ...
    public static PacketAdapter getPacketAdapter() {
        ...
        try {
            return new RuleBasedPacketAdapter()
                    .deserialize(IOUtil.getSharedByteBuf().readStreamFully(new FileInputStream(file)));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
