...
public class FastBiomeDecorator extends BiomeDecorator {
    ...
    private void testMyOreGen(World world) {
        ChunkGeneratorSettings cps = chunkProviderSettings = ChunkGeneratorSettings.Factory.jsonToFactory(world.getWorldInfo().getGeneratorOptions()).build();

        dirtGen = new WGFastOre(Blocks.DIRT.getDefaultState(), cps.dirtSize);
        gravelOreGen = new WGFastOre(Blocks.GRAVEL.getDefaultState(), cps.gravelSize);
        graniteGen = new WGFastOre(Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.GRANITE), cps.graniteSize);
        dioriteGen = new WGFastOre(Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.DIORITE), cps.dioriteSize);
        andesiteGen = new WGFastOre(Blocks.STONE.getDefaultState().withProperty(BlockStone.VARIANT, BlockStone.EnumType.ANDESITE), cps.andesiteSize);
        coalGen = new WGFastOre(Blocks.COAL_ORE.getDefaultState(), cps.coalSize);
        ironGen = new WGFastOre(Blocks.IRON_ORE.getDefaultState(), cps.ironSize);
        goldGen = new WGFastOre(Blocks.GOLD_ORE.getDefaultState(), cps.goldSize);
        redstoneGen = new WGFastOre(Blocks.REDSTONE_ORE.getDefaultState(), cps.redstoneSize);
        diamondGen = new WGFastOre(Blocks.DIAMOND_ORE.getDefaultState(), cps.diamondSize);
        lapisGen = new WGFastOre(Blocks.LAPIS_ORE.getDefaultState(), cps.lapisSize);
    }
    ...
    protected void genDecorations(Biome b, World w, Random rnd) {
        ...
        if (rnd.nextInt(32) == 0 &&
                TerrainGen.decorate(w, rnd, cp, PUMPKIN)) {
    ...
    protected void genStandardOre1(World w, Random rnd, int cnt, WorldGenerator gen, int min, int max) {
        ...
        while (cnt-- > 0) {
            gen.generate(w, rnd, pmb.setPos(pos.getX()+rnd.nextInt(16),
                                            pos.getY()+rnd.nextInt(max)+min,
                                            pos.getZ()+rnd.nextInt(16)));
        }
        pmb.release();
    }
    ...
    protected void genStandardOre2(World w, Random rnd, int cnt, WorldGenerator gen, int center, int dy) {
        ...
        while (cnt-- > 0) {
            gen.generate(w, rnd, pmb.setPos(pos.getX()+rnd.nextInt(16),
                                            pos.getY()+rnd.nextInt(dy)+rnd.nextInt(dy) + center,
                                            pos.getZ()+rnd.nextInt(16)));
        }
        pmb.release();
    }
