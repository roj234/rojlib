...
public abstract class StormHandler {
    ...
    protected static void addRainBlock(RainBlock rb) {
        RainBlock replaced = rainBlocks.addLast(rb);
        if (null != replaced) {
    ...
    public static void onTick(ClientTickEvent event) {
        ...
        if (w != null && w.provider instanceof Climated && !ClientProxy.mc.isGamePaused()) {
            ...
            if (!rainBlocks.isEmpty()) {
                // noinspection all
                while (rainBlocks.peekFirst().life == 0) rainBlocks.removeFirst();
            }
                ...
                if (strength > 0) {
                    ...
                    if (strength > 0) {
                        ...
                        for (int i = 0; i < pass; ++i) {
                            // noinspection all
                            pos.setPos(entity.posX + rnd.nextInt(PARTICLE_RADIUS) - rnd.nextInt(PARTICLE_RADIUS),
                                       entity.posY,
                            ...
                            if (pos.getY() <= entity.posY + PARTICLE_RADIUS && pos.getY() >= entity.posY - PARTICLE_RADIUS &&
                                sp.isVisibleIn(w.getBiome(pos))) {
            ...
            for (int x = posX - r; x <= posX + r; ++x) {
                ...
                if (isVisibleIn(biome)) {
                    ...
                    for (int i = (int) Math.floor(rnd.nextGaussian() * lastStrength); i >= 0; i--) {
                        Vec3d wind = this.tmp.set(tmp).mul(t * rnd.nextDouble());
    ...
    public Vec3d getEntityVelocity(Entity entity, BlockPos.MutableBlockPos pos, Vec3d dest) {
        ...
        dest.mul(1 / region)
            .mul(lastStrength * lastStrength / 50 / ((entity.motionX * entity.motionX + entity.motionY * entity.motionY + entity.motionZ * entity.motionZ) + 10));
        ...
        if (entity instanceof EntityPlayer) {
            ...
            return player.isSneaking() && player.onGround ? dest.mul(0.7) : dest;
        }
    ...
    protected void spawnParticle(World world, double pX, double pY, double pZ) {
        if (rnd.nextDouble() > 0.85) world.spawnParticle(EnumParticleTypes.DRIP_WATER, pX, pY, pZ, 0, 1, 0, EmptyArrays.INTS);
        else if (rnd.nextDouble() > 0.6) world.spawnParticle(EnumParticleTypes.WATER_DROP, pX, pY, pZ, 0, 1, 0, EmptyArrays.INTS);
    }
