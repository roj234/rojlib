...
public class GuiEditPA extends GuiBaseNI implements ComponentListener {
    ...
    public void actionPerformed(Component c, int action) {
        switch (c.getMark()) {
            case 1:
                ...
                break;
            case 2:
                GPopup popup = addPopup("输入框", "",
                                        Collections.singletonList("保存"), 4);
    ...
    private static void parsePacketCode(String data, List<RPACategory> cat) {
        ...
        mySpec.addAll(AbstLexer.SPECIAL);
        mySpec.remove('.');

        int state = 0, dir = 0, id = 0;
        for (String line : new SimpleLineReader(data)) {
            line = line.trim();
                    ...
                    while (i < line.length()) {
                        ...
                        if (mySpec.contains(c)) {
                            ...
                            if (val.endsWith("class")) {
                                cat.get((dir - 1) * 2 + state - 1).replacements.add(new String[]{
    ...
    private static class RPACategory extends GScrollView implements ComponentListener {
        ...
        public RPACategory(IGui parent, IntMap<Target> map,
                           EnumConnectionState state, EnumPacketDirection dir) {
        ...
        protected void addElements(int from, int to) {
            ...
            for (int i = from; i < to; i++) {
                ...
                if (t != null) {
                    components.add(new GButtonNP(this, 6, y, halfWidth, 16, t.toString())
                                  .setDummy());
        ...
        private Target reverseFind(int i) {
            for (IntMap.Entry<Target> entry : map.entrySet()) {
                Target v = entry.getValue();
                if (v instanceof T_IdRemap && ((T_IdRemap) v).toId == i)
                    return v;
            }
            return null;
        }
        ...
        public void actionPerformed(Component c, int action) {
            ...
            switch (c.getMark()) {
                case 998:
                    ...
                    if (selected == -1) {
                        map.put(popup.getMark(), new T_Ignore());
                    } else {
                        String[] nameid = replacements.remove(selected);
                        selected = Integer.parseInt(nameid[1]);
                        if (outside) {
                            map.put(popup.getMark(), new T_IdRemap(popup.getMark(), selected));
                        } else {
                            map.put(selected, new T_IdRemap(selected, popup.getMark()));
                        }
                        refresh();
                    }
                    break;
    ...
    private static class ReplacementPopup extends GScrollView implements ComponentListener {
        ...
        protected void addElements(int from, int to) {
            ...
            int y = 4;
            for (int i = from; i < to; i++) {
                GButton btn = new GButtonNP(this, 4, y, width - 16, 14, data.get(i)[0])
                        .setDummy();
