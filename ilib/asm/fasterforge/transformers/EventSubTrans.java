...
import roj.asm.frame.FrameType;
...
public class EventSubTrans implements ContextClassTransformer {
    ...
    public void transform(String name, Context ctx) {
        if (!name.equals("net.minecraftforge.fml.common.eventhandler.Event") &&
                !name.startsWith("net.minecraft.") && name.indexOf(46) != -1) {
            ...
            Class<?> p = null;
            try {
                p = Launch.classLoader.findClass(superClass.replace('/', '.'));
    ...
    private boolean buildEvents(ConstantData data) {
        ...
        if (!hasResult || !hasCancelable) {
            ...
            for (int i = 0; i < anns.size(); i++) {
                ...
                if (!hasResult && ann.clazz.equals("Lnet/minecraftforge/fml/common/eventhandler/Event$HasResult;")) {
                    ...
                    edited = true;
                } else if (!hasCancelable && ann.clazz.equals("Lnet/minecraftforge/fml/common/eventhandler/Cancelable;")) {
        ...
        if (hasSetup) {
            ...
            insn.add(new IfInsnNode(IFNONNULL, inited));
            ...
            //  same #0xb1 返回, Event
            Frame f = new Frame(FrameType.same, lastRet);
            f.locals.add(new Var(data.name));
