...
class SeparateDismount extends EntityPlayerSP {
    ...
    public void onUpdate() {
        if (world.isChunkGeneratedAt((int) (posX + 0.5) >> 4,
                                     (int) (posZ + 0.5) >> 4)) {
            super_onUpdate();
            if (isRiding()) {
                connection.sendPacket(new Rotation(rotationYaw, rotationPitch, onGround));
                connection.sendPacket(new CPacketInput(moveStrafing, moveForward, movementInput.jump,
                                                       KeyRegister.keyDismount.isKeyDown()));
