 ...
 */
public abstract class AnnVal extends CEntry {
	public static void serialize(CEntry entry, DynByteBuf w, ConstantPool pool) {
		var serializer = new ToJVMAnnotation();
		serializer.init(pool, w);
		entry.accept(serializer);
	}

	public static CEntry valueOf(Type type) {return new AClass(type);}
	public static CEntry ofEnum(String name, String name1) {return new AEnum(name, name1);}

	public static final char STRING = 's', ENUM = 'e', ANNOTATION_CLASS = 'c', ANNOTATION = '@';

	public static CEntry parse(ConstantPool pool, DynByteBuf r) {
		...
		switch (type) {
			...
			case ARRAY:
				...
				List<CEntry> annos = new ArrayList<>(len);
	...
	@Override public final roj.config.data.Type getType() {return roj.config.data.Type.OTHER;}
