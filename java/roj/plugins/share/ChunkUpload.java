...
import roj.http.server.HPostHandler;
...
public class ChunkUpload {
    public static final class Task {
		...
		MyBitSet bitmap;
		int fragmentSize, fragmentCount;
		...
		synchronized FileChannel getFc() throws IOException {return fds.isEmpty() ? FileChannel.open(tmp.toPath(), StandardOpenOption.WRITE) : fds.pop();}
	}
	...
	public Response newTask(File path, String name, long size, @Nullable BiConsumer<Task, Boolean> callback) {
		...
		noCreation: {
			...
			int fragmentCount = (int) ((size+fragmentSize-1) / fragmentSize);
			...
			try {
				...
				return Response.internalError("任务创建失败", e);
			}
		}
		...
		return Response.json("{\"ok\":true,\"id\":\""+getId(task)+"\",\"fragment\":"+fragmentSize+"}");
	}
	...
	public void uploadProcessor(Request req, PostSetting setting) throws IOException {
		...
		setting.postHandler(new HPostHandler() {
			@Override
			...
			public void onSuccess() throws IOException {
				...
				throw new IllegalRequestException(200, Response.text(msg));
			}
