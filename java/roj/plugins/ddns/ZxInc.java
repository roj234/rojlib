...
public class ZxInc extends IpGetter {
	...
	public InetAddress[] getAddress(boolean checkV6) throws Exception {
		CMap url = parser.parse(pooledRequest(new URL("https://v4.ip.zxinc.org/info.php?type=json")).stream()).asMap();
		InetAddress ipv4 = InetAddress.getByName(url.getDot("data.myip").asString());

		InetAddress ipv6;
		if (checkV6) {
			url = parser.parse(pooledRequest(new URL("https://v6.ip.zxinc.org/info.php?type=json")).stream()).asMap();
