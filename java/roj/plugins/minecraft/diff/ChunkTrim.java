...
public class ChunkTrim {
	...
	public static void createBackup(File worldDir, File backup) {
		...
		MyRegionFile[] data = removeBlockChunk(chunks, whitelist, regions, pool);
		if (data == null) return;
		File entities = new File(backup, "entities");entities.mkdir();
		removeOtherChunk(data, new File(worldDir, "entities"), entities, pool);
		File poi = new File(backup, "poi");poi.mkdir();
		removeOtherChunk(data, new File(worldDir, "poi"), poi, pool);
	}
	public static void createInline(File worldDir) {
		File[] chunks = new File(worldDir, "region").listFiles();
		MyRegionFile[] data = removeBlockChunk(chunks, whitelist, null, pool);
		if (data == null) return;
		removeOtherChunk(data, new File(worldDir, "entities"), null, pool);
		removeOtherChunk(data, new File(worldDir, "poi"), null, pool);
	}
			...
			asyncPool.submit(() -> {
				...
				try (var rf = new MyRegionFile(mca)) {
					if (isCompress) {
						...
						MyRegionFile newChunk = null;
						try {
							for (int j = 0; j < 1024; j++) {
								if (rf.hasData(j) && !toDelete.contains(j)) {
									if (newChunk == null) newChunk = new MyRegionFile(file1);
