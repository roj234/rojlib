...
sealed class FrpProxy extends H2Stream implements ChannelHandler permits FrpClient.UDP, FrpHostConnection.Client, FrpServerConnection.TCP {
	...
	@Override protected void onHeaderDone(H2Connection man, HttpHead head, boolean hasData) throws IOException {
		...
		LOGGER.info("流{}已开启({})", this, head.getField(":status"));
	}
	@Override protected void onRST(H2Connection man, int errno) {
		this.man = (FrpCommon) man;
		LOGGER.info("流{}已关闭({})", this, errno);
		try {
			//if (connection.isTCP())
	...
	@Override public String toString() {
		if (man.isServer()) return "["+NetUtil.toString(man.channel().remoteAddress()) +"/#"+id+"]  "+port;
		else return "["+NetUtil.toString(man.channel().localAddress())+"/#"+id+"]  "+NetUtil.toString(connection.remoteAddress())+" => "+port;
	}
