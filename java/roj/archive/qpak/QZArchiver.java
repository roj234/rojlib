...
public class QZArchiver {
	...
	private final List<WordBlockAppend> appends = new ArrayList<>();
	...
	public File compress(TaskPool pool, EasyProgressBar bar) throws IOException {
	...
	private void writeBlock(EasyProgressBar bar, WordBlockAppend block, QZWriter writer, TaskPool canSplit) throws IOException {
		...
		for (int i = 0; i < file.length; i++) {
			...
			if (canSplit != null && canSplit.busyCount() <= autoSplitTaskThreshold) {
