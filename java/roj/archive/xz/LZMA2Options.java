...
public class LZMA2Options implements Cloneable {
	...
	public LZMA2Options setPreset(int preset) {
		...
		if (preset <= 3) {
			...
			mode = MODE_NORMAL;
			mf = MF_BT4;
			niceLen = (preset == 4) ? 16 : (preset == 5) ? 32 : preset < 9 ? 64 : NICE_LEN_MAX;
			depthLimit = 0;
		}
	...
	public void setAsyncMode(TaskExecutor executor, BufferPool bufferPool, LZMA2ParallelManager parallel) {
		asyncExecutor = executor;
		asyncBufferPool = bufferPool;
		asyncMan = parallel;
	}
