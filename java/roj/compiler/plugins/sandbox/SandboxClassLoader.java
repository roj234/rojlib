...
class SandboxClassLoader extends ClassLoader {
	private MyHashSet<String> allowed = new MyHashSet<>("java.lang", "java.util", "java.util.regex", "java.util.function", "java.text", "roj.compiler.plugins.sandbox", "roj.reflect", "roj.compiler", "roj.asm.cp", "roj.asm.tree.anno");
	...
	public SandboxClassLoader(ClassLoader parent, MyHashMap<String, byte[]> data) {
		super(parent);
		classData = data;
	}
		...
		synchronized (getClassLoadingLock(name)) {
			...
			if (c == null) {
				try {
					...
					if (!allowed.contains(pkg)) new SecurityException("这个类不在白名单中"+name).printStackTrace();
