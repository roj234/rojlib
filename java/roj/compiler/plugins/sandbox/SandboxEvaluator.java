 ...
 */
public interface SandboxEvaluator {
	public static void init(GlobalContext ctx, AnnotationRepo repo) throws IOException {
		MyHashMap<String, byte[]> data = new MyHashMap<>();
		SimpleList<RawNode> invoker = new SimpleList<>();

		for (AnnotatedElement element : repo.annotatedBy("roj/compiler/api/Constant")) {
			if (element.isLeaf()) {
		...
		for (int j = 0; j < invoker.size(); j++) {
			...
			info.methods().get(i).putAttr(new SandboxMethod(evaluator, j, TypeHelper.parseReturn(mof.rawDesc())));
		}
	}
