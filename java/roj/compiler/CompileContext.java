...
import static roj.compiler.Tokens.rParen;
...
public class CompileContext {
	...
	public IType transformPseudoType(IType type) {
		if (type.owner() != null && !"java/lang/Object".equals(type.owner())) {
			var info = compiler.resolve(type.owner());
			if (info.parent() == null) {
	...
	public void report(Kind kind, String message) {
		...
		compiler.report(file, kind, errorReportIndex >= 0 ? errorReportIndex : lexer.index, message);
	}
	public void report(Kind kind, String message, Object... args) {
		...
		compiler.report(file, kind, errorReportIndex >= 0 ? errorReportIndex : lexer.index, message, args);
	}
	public void report(Expr node, Kind kind, String message) {
		if (errorCapture != null) {errorCapture.accept(message, ArrayCache.OBJECTS);return;}
		compiler.report(file, kind, node.getWordStart(), node.getWordEnd(), message, ArrayCache.OBJECTS);
	}
	public void report(Expr node, Kind kind, String message, Object... args) {
		...
		if (errorCapture != null) {errorCapture.accept(message, args);return;}
		compiler.report(file, kind, node.getWordStart(), node.getWordEnd(), message, args);
	}
	public void report(int knownPos, Kind kind, String message, Object... args) {
		compiler.report(file, kind, knownPos, message, args);}
	//endregion
	...
	private String reportSimilarField(ClassDefinition type, String field) {
		...
		for (var entry : compiler.link(type).getFields(compiler).entrySet()) {
			if (TextUtil.editDistance(field, entry.getKey()) < (field.length()+1)/2) {
				similar.add(entry.getKey());
			}
		}
		if (similar.isEmpty()) return "";
	...
	// 这个对象在递归解析中继承
	public ArrayList<NestContext> enclosing = new ArrayList<>();
	 ...
	 */
	public IType writeCast(MethodWriter cw, Expr expr, IType returnType) {
		IType realSourceType = expr.minType();
		var cast = caster.checkCast(realSourceType, returnType);
		...
		if (cast.type < 0) {
			var override = getOperatorOverride(expr, returnType, rParen);
			if (override == null) {
				report(Kind.ERROR, "typeCast.error."+cast.type, realSourceType, returnType);
				return returnType;/*Might be <Unresolved>*/
			} else {
				override.write(cw);
	...
	public static CompileContext next() {
		var ctx = FTL.get();
		if (ctx.next == null) {
			var next = ctx.compiler.createContext();
