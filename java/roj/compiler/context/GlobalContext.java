...
public class GlobalContext implements LavaFeatures {
	...
	private static final XashMap.Builder<ClassDefinition, ResolveHelper> CLASS_EXTRA_INFO_BUILDER = XashMap.builder(ClassDefinition.class, ResolveHelper.class, "owner", "_next", Hasher.identity());

	protected final XashMap<String, CompileUnit> ctx = COMPILE_UNIT_BUILDER.create();
	...
	protected final XashMap<ClassDefinition, ResolveHelper> extraInfos = CLASS_EXTRA_INFO_BUILDER.create();
	...
	//endregion
	//region ResolveHelper基础
	@NotNull public synchronized final ResolveHelper getResolveHelper(ClassDefinition info) { return extraInfos.computeIfAbsent(info); }
	public synchronized void invalidateResolveHelper(ClassDefinition file) {extraInfos.removeKey(file);}

	@NotNull public ToIntMap<String> getHierarchyList(ClassDefinition info) {return getResolveHelper(info).getHierarchyList(this);}
	@NotNull public ComponentList getMethodList(ClassDefinition info, String name) {return getResolveHelper(info).getMethods(this).getOrDefault(name, ComponentList.NOT_FOUND);}
	@NotNull public ComponentList getFieldList(ClassDefinition info, String name) {return getResolveHelper(info).getFields(this).getOrDefault(name, ComponentList.NOT_FOUND);}
	@Nullable public List<IType> getTypeArgumentsFor(ClassDefinition info, String superType) {return getResolveHelper(info).getTypeParamOwner(this).get(superType);}
	@NotNull public Map<String, InnerClasses.Item> getInnerClassInfo(ClassDefinition info) {return getResolveHelper(info).getInnerClasses(this);}
	public AnnotationType getAnnotationDescriptor(ClassDefinition info) {return getResolveHelper(info).annotationInfo();}
	...
	public final List<IType> inferGeneric(@NotNull IType typeInst, @NotNull String targetType, @Nullable Map<?, ?> temp) {
		var info = getClassInfo(typeInst.owner());
		...
		if (typeInst instanceof Generic g) {
			...
			if (g.children.size() == 1 && g.children.get(0) == Asterisk.anyGeneric) {
				...
				info = getClassInfo(targetType);
				var sign = info.getAttribute(info.cp, Attribute.SIGNATURE);

				MyHashMap<String, IType> realType = temp==null?new MyHashMap<>():Helpers.cast(temp);
	...
	private List<IType> inferGeneric0(ClassNode typeInst, List<IType> params, String target) {
		...
		for(;;) {
			...
			for(;;) {
				...
				typeInst = getClassInfo(type.owner());
	...
	public final boolean instanceOf(String testClass, String instClass) {
		ClassDefinition info = getClassInfo(testClass);
	...
	public final IType getCommonParent(IType a, IType b) {
		...
		ClassDefinition infoA = getClassInfo(a.owner());
		if (infoA == null) return a; // should be checked by LocalContext#resolveType
		ClassDefinition infoB = getClassInfo(b.owner());
		if (infoB == null) return a; //

		String commonParent = getCommonParent(infoA, infoB);
		assert commonParent != null;

		var info = getClassInfo(commonParent);
		...
		for (int i = 0; i < aGeneric.size(); i++) {
			...
			IType c = aa.equals(a) && bb.equals(b) ? Signature.any() : getCommonParent(aa, bb);
			typeParams.set(i, c);
		}
	 ...
	 * 返回ab两个类(Class)的共同父类
	 */
	public final String getCommonParent(ClassDefinition infoA, ClassDefinition infoB) {
		ToIntMap<String> tmp,
	...
	//region createThreadLocalObject
