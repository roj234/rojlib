...
public final class JavaCompileUnit extends CompileUnit {
	...
	public boolean S1parseStruct() throws ParseException {
		...
		while (w.type() == IMPORT) {
			...
			if (tmp.charAt(tmp.length()-1) == '*') {
				...
				HashMap<String, String> map = !impField ? importList.getImportClass() : importList.getImportStatic();
	...
	private void header(int acc) throws ParseException {
		...
		if (ctx.compiler.hasFeature(Compiler.VERIFY_FILENAME) && (acc&ACC_PUBLIC) != 0 && _parent == this && !IOUtil.fileName(filename).equals(w.val())) {
	...
	private void enumFields(Tokens wr, HashSet<String> names, CompileContext ctx) throws ParseException {
	...
	// endregion
	private void ADTFields(Tokens wr, HashSet<String> names, CompileContext ctx) throws ParseException {
		assert fields.isEmpty();
