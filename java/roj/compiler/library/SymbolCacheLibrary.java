...
public final class SymbolCacheLibrary extends ClassLoaderLibrary {
	...
	private static List<Library> init() {
		String property = System.getProperty("roj.compiler.symbolCache", ".lavaCache/LocalSymbolCache.zip");
		if (property.equals("DISABLE")) return Collections.emptyList();
		...
		var symTable = new File(property);
		if (symTable.isFile()) {
			try {
