...
public class LavaCompiler extends Resolver implements Compiler {
	...
	public LavaCompiler(boolean _dontAddSystemRuntime) {super(_dontAddSystemRuntime);}
	...
	public CompileContext createContext() {
		...
		ctx.lexer.literalEnd(myJavaLend).tokenIds(myJavaToken);
		ctx.lexer.literalState = myJavaLst;
	...
	protected static final int TF_ANYARGC = 1, TF_PRIMITIVE = 2, TF_NORAW = 4;
	/**
	 * 解析时忽略这个类的泛型限制，用于某些编译器内部类，需要在泛型中保存可变参数
	 */
	@MagicConstant(flags = {TF_ANYARGC, TF_PRIMITIVE, TF_NORAW})
	...
	private void addAnnotations(Library library) {
		if (library instanceof JarLibrary lib) {
			try {
				annotations.addOptionalCache(lib.jar);
			} catch (IOException e) {
				e.printStackTrace();
			}
		} else {
	...
	private final BitSet myJavaLend = Tokens.getLiteralEnd();
	private final BitSet[] myJavaLst = Tokens.getLiteralState();
	private int nextTokenId = Tokens.getInitialTokenCount();

	@Override public int tokenCreate(String token, int tokenCategory) {
		Word word = myJavaToken.putIfAbsent(token, new Word().init(nextTokenId, 0, token));
	...
	@Override public void tokenAlias(int token, String alias) {myJavaToken.put(alias, new Word().init(token, 0, alias));}
	...
	@Override public void onUnary(String operator, Type type, MethodNode node, int side) {operators.computeIfAbsentInt(tokenId(operator), x -> new ArrayList<>()).add(SimpleOp(type, null, node));}
	@Override public void onBinary(Type left, String operator, Type right, MethodNode node, boolean swap) {operators.computeIfAbsentInt(tokenId(operator), x -> new ArrayList<>()).add(SimpleOp(left, right, node));}
	@Override public void addOpHandler(String operator, ExprOp resolver) {operators.computeIfAbsentInt(tokenId(operator), x -> new ArrayList<>()).add(resolver);}
	//endregion
	//region 沙盒API
