...
public final class MethodParser {
	...
	private void statement(Word w) throws ParseException {
		...
		for(;;) {
		switch (w.type()) {
			case Word.EOF -> throw wr.err("未预料的文件终止");
			...
			default -> {
				...
				if (w.type() == Word.LITERAL) {
					...
					if (w.val().equals("var") && varConst(false)) break;
	...
	private void unpackReturnStack(boolean isFinal) throws ParseException {
		...
		if (w.type() != rBrace) throw wr.err("unexpected_2:[\""+w.val()+"\",block.except.multiReturn]");
			...
			if (w.type() == assign) {
				...
				if (needInfer) {
					...
					if (type instanceof Generic g && g.children.size() == 1 && g.children.get(0) == Asterisk.anyGeneric) {
						ClassNode info = ctx.compiler.resolve(type.owner());
