...
public class BitSet implements Iterable<Integer> {
	...
	public BitSet() { this(1); }
	public BitSet(int size) {
		this.set = new long[(size >> 6) + 1];
		this.cap = set.length << 6;
	}

	public BitSet(long[] set, int count) {
		this.set = set;
		this.cap = set.length << 6;
		this.size = count;
		if (count > 0) {
    ...
    public static BitSet from(CharSequence data) {
		BitSet set = new BitSet(data.length());
	...
	public static BitSet from(byte... data) {
		BitSet set = new BitSet(data.length);
		for (byte i : data) {
			set.add(i);
		}
	...
	public static BitSet from(int... data) {
		BitSet set = new BitSet(data.length);
		for (int i : data) {
			set.add(i);
		}
	...
	public static BitSet fromRange(int from, int to) {
	...
	public final BitSet and(BitSet o) {
		if (o.max < 0) {
			clear();
			return this;
		}
	...
	public final BitSet or(BitSet o) {
	...
	public final BitSet xor(BitSet o) {
		if (o.max < 0) return this;
	...
	public final BitSet addAll(CharSequence s) {
	...
	public Throwable x;
	public final BitSet copy() {return copyTo(new BitSet(cap-1));}
	public final BitSet copyTo(BitSet copied) {
		copied.expand(cap);
	...
	public boolean equals(Object obj) {
		if (obj instanceof BitSet mbs && mbs.size == this.size && mbs.max == this.max) {
			int size = max+1;
			int i = 0;
