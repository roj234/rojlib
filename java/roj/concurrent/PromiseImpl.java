 ...
 */
final class PromiseImpl<T> implements Promise<T>, Task, Promise.Callback {
	PromiseImpl() {}
	...
	private TaskExecutor executor, rejectExecutor;
	...
	@Override public final Promise<T> rejectedAsync(TaskExecutor executor, Function<Throwable, T> recover) {
		this.rejectExecutor = executor;
		return setRejectHandler(recover);
	}
	@Override public final Promise<T> rejectedComposeAsync(TaskExecutor executor, Function<Throwable, T> recover) {
		this.rejectExecutor = executor;
		return setRejectHandler(recover);
	}
	...
	private void _apply() {
		...
		if ((state&TASK_SUCCESS) != 0) {
			...
			while (promise != null) {
				...
				if (U.compareAndSwapInt(promise, STATE, pState, pState|TASK_COMPLETE|INVOKE_HANDLER))
	...
	private boolean notifyNext(int state) {
		if ((state&CALLBACK) != 0 && U.compareAndSwapInt(next, STATE, next._state&(CALLBACK|WAIT), state)) {
			do {
				state = _state;
			} while (!U.compareAndSwapInt(this, STATE, state, state & ~CALLBACK));
	...
	private boolean once(int flag) {
		while (true) {
			...
			if (U.compareAndSwapInt(this, STATE, st, st|flag)) return true;
		}
	}
	...
	public void resolve(Object value) {
		if (value instanceof PromiseImpl<?> waitFor) {
			...
			if (U.compareAndSwapInt(this, STATE, myState, myState|WAIT)) {
				while (true) {
					...
					if ((val & CALLBACK) != 0) {
						...
						if (U.compareAndSwapInt(waitFor, STATE, val, val|CALLBACK)) {
	...
	@Override public void resolveOn(Object value, TaskExecutor executor1) {
		executor = executor1;
		resolve(value);
	}
	@Override public void reject(Throwable reason) {finish(TASK_COMPLETE, reason);}
	@Override public void rejectOn(Throwable reason, TaskExecutor executor1) {
		executor = executor1;
		reject(reason);
	}
	...
	private void finish(int target, Object value) {
		...
		if (U.compareAndSwapInt(this, STATE, s, s|target)) _val = value;
		_apply();
	}
	// endregion
