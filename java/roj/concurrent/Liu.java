...
public interface Liu<T> {
	...
	static <T> Liu<T> empty() {return c -> {};}
	static <T> Liu<T> of(T t) {return c -> c.accept(t);}
	@SafeVarargs static <T> Liu<T> of(T... t) {return of(Arrays.asList(t));}
	static <T> Liu<T> of(Iterable<T> t) {return t::forEach;}
	static Liu<String> linesOf(File file) throws IOException {
		return c -> {
	...
	default <R> Liu<R> map(Function<T, R> mapper) {return c -> consume(t -> c.accept(mapper.apply(t)));}
	default <R> Liu<R> map(Function<T, R> mapper1, int count, Function<T, R> mapper2) {
		return c -> {
	...
	default <R> Liu<R> flatMap(Function<T, Liu<R>> mapper) {return c -> consume(t -> mapper.apply(t).consume(c));}
	default Liu<T> filter(Predicate<T> filter) {
		return c -> consume(t -> {
			if (filter.test(t)) {
				c.accept(t);
			}
	...
	default Liu<T> andThen(Consumer<T> consumer) {return c -> consume(consumer.andThen(c));}
	default <U, R> Liu<R> zip(Iterable<U> newParam, BiFunction<T, U, R> combiner) {
		return c -> {
	...
	class ShuzuLiu<T> extends ArrayList<T> implements Liu<T> {
		@Override public void consume(Consumer<T> consumer) {forEach(consumer);}
	}
	default Liu<T> cached() {
		var arraySeq = new ShuzuLiu<T>();
		consume(arraySeq::add);
		return arraySeq;
	}
	default Liu<T> parallel() {
