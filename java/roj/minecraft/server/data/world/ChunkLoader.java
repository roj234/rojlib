...
public final class ChunkLoader implements Closeable {
	...
	private final LRUCache<MutableInt, MyRegionFile> chunks = new LRUCache<>(50, 5);

	private MutableInt _id = new MutableInt();
	...
	private synchronized MyRegionFile getRegion(int x, int z) throws IOException {
		MutableInt _id = this._id;
		if (_id == null) _id = new MutableInt();
		else this._id = null;
		...
		if (region == null) {
			...
			chunks.put(new MutableInt(id), region);
		}
