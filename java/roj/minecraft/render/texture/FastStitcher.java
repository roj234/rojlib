...
public class FastStitcher {
	private final PriorityQueue<Holder> holders = new PriorityQueue<>();
	...
	public void stitch() {
		...
		Object[] array = PriorityQueueHelper.INSTANCE.getInternalArray(holders);
		for (int i = 0; i < holders.size(); i++) {
			Holder h = (Holder) array[i];
			if (!fitIn(h)) {
				for (int j = 0; j < i; j++) {
	...
	public List<IAtlasPiece> getRegisteredTextures() {
		...
		Object[] array = PriorityQueueHelper.INSTANCE.getInternalArray(holders);
		for (int i = 0; i < holders.size(); i++) list.add(((Holder) array[i]).getAtlasSprite());
		return list;
	}
