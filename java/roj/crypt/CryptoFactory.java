...
public final class CryptoFactory extends Provider {
	...
	//end 流密码
	//AEAD
	public static RCipherSpi AES_GCM() {return new AES_GCM();}
	public static RCipherSpi ChaCha_Poly1305() {return new ChaCha_Poly1305(new ChaCha());}
	public static RCipherSpi XChaCha_Poly1305() {return new ChaCha_Poly1305(new XChaCha());}
	//end AEAD
	...
	//伪随机数生成器
	@Deprecated public static Random MT19937Random() {return new MT19937();}
	@Deprecated public static Random MT19937Random(long seed) {return new MT19937(seed);}
	...
	public static void AES_SIV_Encrypt(byte[] key,
		...
		aes_ctr.init(RCipherSpi.ENCRYPT_MODE, key, new IvParameterSpecNC(siv), null);
		output.put(siv);
	...
	public static void AES_SIV_Decrypt(byte[] key,
		...
		var aes_ctr = new FeedbackCipher(CryptoFactory.AES(), FeedbackCipher.MODE_CTR);
		aes_ctr.init(RCipherSpi.DECRYPT_MODE, key, new IvParameterSpecNC(siv), null);
	...
	public static byte[] HKDF_HmacSha256(byte[] inputKeyMaterial, @Nullable byte[] salt, int outputLength) {
		try {
			return HKDF_expand(
					new HMAC(MessageDigest.getInstance("SHA-256")),
					inputKeyMaterial,
