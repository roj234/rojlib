...
public class DerReader {
	...
	public DerValue readInt(int length) throws IOException {
		byte[] data;
		if (length > 65536) {
		...
		return new DerValue.Int(bi);
	}

	public DerValue readBits(int length) throws IOException {
		int trash = in.readUnsignedByte();
		if (trash >= 8) throw new CorruptedInputException("bit string "+trash);
		return new DerValue.Bit(trash, readDirectBytes(length-1));
	}

	public DerValue readBytes(int length) throws IOException {
		return new DerValue.Bytes(readDirectBytes(length));
	}

	public DerValue readOid(int length) throws IOException {
		...
		return new DerValue.OID(oids.toArray());
	}

	public DerValue readStr(int type, int len) throws IOException {
		String str = len > 65536 ? new String(safeRead(len), StandardCharsets.ISO_8859_1) : in.readAscii(len);
		return new DerValue.Str(type, str);
	}
	public DerValue readUTF(int len) throws IOException {
		String str = len > 65536 ? new String(safeRead(len), StandardCharsets.UTF_8) : in.readUTF(len);
		return new DerValue.Str(DerValue.UTF8_STRING, str);
	}

	public DerValue readUnp(int type, int length) throws IOException {
		return new DerValue.Unk(type, readDirectBytes(length));
	}

	public byte[] readDirectBytes(int length) throws IOException {
		byte[] data;
		if (length > 65536) {
			data = safeRead(length);
		} else {
