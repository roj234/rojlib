...
public final class MySaltedHash {
	...
	public String hash(byte[] pass) {
		...
		DirectByteList tmp = rawHash(pass, salt, cost, 20-cost);
	...
	public boolean compare(CharSequence str, byte[] pass) {
		...
		DirectByteList myHash = rawHash(pass, tmp.toByteArray(), cost, 20-cost);

		tmp.clear();
		DynByteBuf dbHash = Base64.decode(hash1.get(4), tmp, Base64.B64_URL_SAFE_REV);
