...
final class EventListenerImpl implements EventListener {
	...
	private ASM asm() {
		...
		IType type;
		if (sign != null && (type = sign.values.get(0)) instanceof Generic g) {
			cw.visitSizeMax(2, 0);
		...
		if (infos.size() > 1) {
			...
			for (int i = 0; i < infos.size(); i++) {
				if (EventBus.isDebugMode()) {
					cw.ldc(i);
					cw.invokeS("roj/asmx/event/EventBus", "setEventListenerId", "(I)V");
				}

				MethodNode mn = infos.get(i).mn;
			...
			if (EventBus.isDebugMode()) {
				String debugMagic = info.mn.ownerClass().replace('/', '.')+"#"+info.mn.name();
				CodeWriter cw1 = c.newMethod(ACC_PUBLIC|ACC_FINAL, "toString", "()Ljava/lang/String;");
				cw1.visitSize(1, 1);
				cw1.ldc(debugMagic);
				cw1.one(ARETURN);
			}
	...
	private static int callEvent(ConstantData out, CodeWriter c, MethodNode listener, int fid) {
