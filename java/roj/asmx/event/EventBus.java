...
public class EventBus {
	...
	public static synchronized void registerEvent(Class<? extends Event> type) {}
	...
	// 调试frozen event的时候也许用到？ ThreadLocal<MutableInteger>
	public static void setEventListenerId(int id) {}
	public static boolean isDebugMode() { return NativeLibrary.EXTRA_BUG_CHECK; }
	...
	private ListenerList createListenerList(Class<?> c, MethodNode mn, String event) {
		Class<?> type = null;
		try {
	...
	public EventBus(AnnotationRepo repo) {
		...
		for (AnnotatedElement el : repo.annotatedBy(SUBSCRIBE_NAME)) {
			...
			for (AnnotatedElement.Node child : el.parent().children()) {
				...
				if (subscribe != null) {
					MethodNode mn = (MethodNode) child.node();
	...
	private ListenerInfo[] getListenerInfo(Class<?> type, boolean object) {
		...
		if (entry == null) {
			ConstantData data = Parser.parse(type);
	...
	private static void toListenerInfo(MethodNode mn, Annotation subscribe, List<ListenerInfo> objectList, List<ListenerInfo> staticList) {
