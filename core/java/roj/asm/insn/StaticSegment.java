...
import static roj.asm.Opcodes.WIDE;
...
public final class StaticSegment extends Segment {
	...
	@Override DynByteBuf getDataSlice() {return array.getClass() == byte[].class ? DynByteBuf.wrap((byte[]) array, off, length) : ((DynByteBuf) array).slice();}
	...
	static boolean isTerminate(DynByteBuf buf, int start, int end) {
		...
		do {
			...
			int length = 0xF & (InsnNode.OPLENGTH[op & 0xFF] >>> 4);
			if (length == 0) throw new IllegalStateException("Illegal instruction "+Opcodes.toString(op));
			if (wide) length *= 2;
			start += length;
		} while (start != end);
