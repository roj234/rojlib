 ...
 */
public final class LZMA2ParallelManager {
	...
	private final ArrayList<Compressor> tasksFree = new ArrayList<>();
	private int taskFree;

	public LZMA2ParallelManager(LZMA2Options options,
								int blockSize,
								@MagicConstant(intValues = {LZMA2Options.ASYNC_DICT_NONE, LZMA2Options.ASYNC_DICT_SET, LZMA2Options.ASYNC_DICT_ASYNCSET}) int dictMode,
								@Range(from = 1, to = 255) int affinity) {
		if (blockSize != 0 && blockSize < LZMA2Options.ASYNC_BLOCK_SIZE_MIN || blockSize > LZMA2Options.ASYNC_BLOCK_SIZE_MAX) throw new IllegalArgumentException("无效的分块大小 "+blockSize);
		if (affinity < 1) throw new IllegalArgumentException("无效的并行任务数量 "+affinity);
		if (dictMode < 0 || dictMode > LZMA2Options.ASYNC_DICT_ASYNCSET) throw new IllegalArgumentException("无效的词典处理模式 "+dictMode);
		...
		if (dictMode == LZMA2Options.ASYNC_DICT_NONE) dictSize = 0;
		...
		int mySize = bufLen;
		if (dictMode == LZMA2Options.ASYNC_DICT_SET) mySize -= options.getDictSize();
		// 6 / 65536 < 1 / 10000
	...
	final void remove(Writer caller, DynByteBuf buffer) {
		...
		synchronized (this) {
			if (--refCnt == 0) {
				for (Compressor encoder : tasksFree) encoder.free();
				tasksFree.clear();
	...
	final void submitTask(Writer caller) throws IOException {
		...
		try {
			task.owner = caller;
			var javac傻逼 = task;
			options.getAsyncExecutor().executeUnsafe(() -> caller.prepareTask(javac傻逼).run());
		} catch (Exception e) {
			Helpers.athrow(e);
		}
	}
	...
	static final class Writer extends OutputStream implements Finishable {
		...
		private final LZMA2ParallelManager man;
		...
		Writer(OutputStream out, LZMA2ParallelManager man) {
		...
		final Task prepareTask(Compressor task) {
			...
			task.id = taskId++;
			task.in.clear();
			if (dictMode == LZMA2Options.ASYNC_DICT_ASYNCSET) {
	...
	static final class Compressor extends LZMA2Out implements Task {
		final LZMA2ParallelManager man;
		DynByteBuf in;
		int pendingSize;
		...
		Compressor(LZMA2ParallelManager _owner) {
		...
		public final void run() throws Exception {
			int off, len;
			if (man.dictMode == LZMA2Options.ASYNC_DICT_ASYNCSET) {
				off = man.dictSize;
				len = pendingSize -= off;
			} else {
				off = 0;
			...
			if (id == 0) {
				...
				switch (man.dictMode) {
					case LZMA2Options.ASYNC_DICT_NONE: state = DICT_RESET; break;
					case LZMA2Options.ASYNC_DICT_ASYNCSET: lzma.lzPresetDict0(off, null, in.address()+man.asyncBufOff, off);
					case LZMA2Options.ASYNC_DICT_SET: state = STATE_RESET; break;
				}
			}
