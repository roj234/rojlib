...
import static roj.archive.xz.LZMA2Out.COMPRESSED_SIZE_MAX;
import static roj.archive.xz.LZMAInputStream.getDictSize;

public class LZMA2ParallelInputStream extends MBInputStream {
	...
	private final ArrayList<SubDecoder> tasksFree = new ArrayList<>();
	...
	final class SubDecoder implements Task {
		...
		public final void execute1() throws InterruptedException, IOException {
			ByteList ob;
			boolean isGlobalOut;
			synchronized (tasksFree) {
				if (doneId == id) {
					isGlobalOut = true;
					ob = LZMA2ParallelInputStream.this.out;
				} else {
					isGlobalOut = false;
					ob = tmpOut;
	...
	private ByteList getDecoder() throws IOException {
		if (decoder == null || decoder.remoteNoMore) {
			...
			taskExecutor.executeUnsafe(task);
	...
	public void close() throws IOException {
		...
		for (SubDecoder decoder : tasksFree) decoder.free();
		tasksFree.clear();
