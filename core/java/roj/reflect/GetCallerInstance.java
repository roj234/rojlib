...
public final class GetCallerInstance {
	...
	static {
		try {
			...
			var localsAndOperands = U.getReference(optionType, Unaligned.fieldOffset(optionType, "LOCALS_AND_OPERANDS"));
	...
	public static Object getCallerInstance(int skip) {
		return LIVE.walk(stream -> {
			...
			return frame == null ? null : ((Object[]) U.getReference(frame, LOCALS))[0];
		});
	}
}
