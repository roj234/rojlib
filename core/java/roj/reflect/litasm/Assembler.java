				...
				loadStack: {
					if (isFloat(type)) {
						...
						if (!type.isPrimitive() && baseOffset(type, annotations[i]) != 0) {
							base = 4;
							buf.preInsert(0, 9);
							buf.putInt(0, 0x4883C100 | baseOffset(type, annotations[i]));// add rcx, #imm8
						} else {
							base = 0;
							buf.preInsert(0, 5);
						}
						buf.putInt(base, 0b01001000_10001001_01_001_100_00100100).put(base+4, stackBottom);// [stack+16] = rcx
						ecxStackPos = base+4;
					} else if (intArgIndex == 5) {
						buf.put(ecxStackPos, buf.get(ecxStackPos) + 8);
