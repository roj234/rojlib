...
public abstract class ObjectMapper {
	...
	public final <T> T read(ConfigValue in, Class<T> type) {
		return reader(type).read(in);
	}
	public final <T> T read(File in, Class<T> type, Parser parser) throws IOException, ParseException {
		return reader(type).read(in, parser);
	}
	public final <T> T read(DynByteBuf in, Class<T> type, Parser parser) throws IOException, ParseException {
		return reader(type).read(in, parser);
	}
	public final <T> T read(InputStream in, Class<T> type, Parser parser) throws IOException, ParseException {
		return reader(type).read(in, parser);
	}
	public final <T> T read(CharSequence in, Class<T> type, TextParser parser) throws ParseException {
		return reader(type).read(in, parser);
	}
	...
	public static <T> IntFunction<T> containerFactory(Class<?> type) {
		...
		synchronized (dc) {
			...
			IntFunction<T> fn = Helpers.cast(Reflection.createInstance(type, c));
