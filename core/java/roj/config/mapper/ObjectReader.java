...
public interface ObjectReader<T> extends ValueEmitter {
	@StaticMethod
	default T read(ConfigValue value) {
		value.accept(reset());
		return get();
	}
	@StaticMethod
	default T read(File in, Parser parser) throws IOException, ParseException {
		parser.parse(in, reset());
		return get();
	}
	@StaticMethod
	default T read(DynByteBuf in, Parser parser) throws IOException, ParseException {
		parser.parse(in, reset());
		return get();
	}
	@StaticMethod
	default T read(InputStream in,  Parser parser) throws IOException, ParseException {
		parser.parse(in, reset());
		return get();
	}
	@StaticMethod
	default T read(CharSequence in, TextParser parser) throws ParseException {
		parser.parse(in, reset());
		return get();
	}
