...
public class YamlParser extends Parser implements StreamParser {
	...
	static {
		...
		YAML_TOKENS.put("#", new Token().init(0, ST_SINGLE_LINE_COMMENT, "#"));
		YAML_TOKENS.put("'''", new Token().init(0, ST_MULTI_LINE_COMMENT, "'''"));
		YAML_TOKENS.put("'", new Token().init(0, ST_LITERAL_STRING, "'"));
		YAML_TOKENS.put("\"", new Token().init(0, ST_STRING, "\""));
	}

	private static void put(int id, String... tokens) {
		for (String token : tokens) {
			Token w = new Token().init(id, 0, token);
		...
		switch (w.type()) {
			case force_cast: {
				...
				switch (cnt) {
					...
					case "int": return ConfigValue.valueOf(val.asInt());
					case "bool": return ConfigValue.valueOf(val.asBool());
					case "long": return ConfigValue.valueOf(val.asLong());
					case "map": return val.asMap();
					case "set":
						MapValue map = val.asMap();
						for (ConfigValue entry1 : map.values()) {
							if (entry1 != NullValue.NULL) throw err("无法转换为set: 值不是null: "+entry1);
						}
						return map;
			...
			case Token.STRING, Token.LITERAL: {
				ConfigValue map = checkMap();
				return map != null ? map : ConfigValue.valueOf(cnt);
			}
			case Token.DOUBLE, Token.FLOAT: {
				double number = w.asDouble();
				ConfigValue map = checkMap();
				return map != null ? map : ConfigValue.valueOf(number);
			}
			case Token.INTEGER: {
				int number = w.asInt();
				ConfigValue map = checkMap();
			...
			case Token.RFCDATE_DATE: return new DateValue(w.asLong());
			case Token.RFCDATE_DATETIME, Token.RFCDATE_DATETIME_TZ: return new TimestampValue(w.asLong());
			case Token.LONG: return ConfigValue.valueOf(w.asLong());
			case TRUE, FALSE: {
				boolean b = w.type() == TRUE;
				ConfigValue map = checkMap();
				return map != null ? map : ConfigValue.valueOf(b);
			}
			case NULL: {
				ConfigValue map = checkMap();
				return map != null ? map : NullValue.NULL;
			}
			case join: {
				ConfigValue map = checkMap();
			...
			case ref: {
				ConfigValue val = anchors.get(cnt);
				if (val == null) throw err("不存在的锚点 "+cnt);
				return val;
			}
			default: unexpected(cnt); // Always fail
			case Token.EOF: return NullValue.NULL;
		}
	}
	...
	public void streamElement(@MagicConstant(flags = LENIENT) int flag, ValueEmitter emitter) throws ParseException {
		...
		try {
			switch (w.type()) {
				...
				case Token.STRING, Token.LITERAL -> {
					var cnt = w.text();
					if (!checkMap(emitter)) emitter.emit(cnt);
				}
				case Token.DOUBLE, Token.FLOAT -> {
					double d = w.asDouble();
					if (!checkMap(emitter)) emitter.emit(d);
				}
				case Token.INTEGER -> {
					int i = w.asInt();
					if (!checkMap(emitter)) emitter.emit(i);
				}
				case Token.RFCDATE_DATE -> emitter.emitDate(w.asLong());
				case Token.RFCDATE_DATETIME, Token.RFCDATE_DATETIME_TZ -> emitter.emitTimestamp(w.asLong());
				case Token.LONG -> emitter.emit(w.asLong());
				case TRUE, FALSE -> {
					boolean b = w.type() == TRUE;
					if (!checkMap(emitter)) emitter.emit(b);
				}
				case NULL -> {
					if (!checkMap(emitter)) emitter.emitNull();
				}
				case delim -> {
					if (prevLN == LN && LN != 1) {
						if ((flag & LENIENT) == 0) throw err("一行内不允许放置多级列表 (你看的不累吗) (通过LENIENT参数关闭该限制)");
				...
				case Token.EOF -> emitter.emitNull();
	...
	private void blockSeq(ValueEmitter emitter) throws ParseException {
		int superIndent = prevIndent;
		int firstIndent = indent;
		if (firstIndent < superIndent) throw err("下级缩进("+firstIndent+")<上级("+superIndent+")");
		...
		while (true) {
			switch (w.type()) {
				...
				case join:
					...
					ConfigValue entry = anchors.get(w.text());
					if (entry == null) throw err("不存在的锚点 "+w.text());
					if (!entry.mayCastTo(Type.MAP)) throw err("锚点 "+w.text()+" 无法转换为map");
				...
				case NULL:
					...
					comment = getComment(comment, name);
	...
	private void map(Token w, ValueEmitter emitter) throws ParseException {
		int superIndent = prevIndent;
		int firstIndent = indent;
		if (firstIndent <= superIndent) throw err("下级缩进("+firstIndent+")<=上级("+superIndent+")");
		...
		while (true) {
			...
			switch (w.type()) {
				case ask: throw err("配置文件不用字符串做key是坏文明");
				case join: throw err("访问者模式不支持前向引用");
				case Token.LITERAL, Token.STRING:
				case Token.INTEGER, Token.LONG, Token.DOUBLE, Token.FLOAT:
				case NULL:
					except(colon, ":");
	...
	private final Token tmpKey = new Token();
	private ConfigValue checkMap() throws ParseException {
		mark();
	...
	private boolean checkMap(ValueEmitter emitter) throws ParseException {
		mark();
	...
	private final HashMap<String, ConfigValue> anchors = new HashMap<>();
	private int indent, prevIndent;
	...
	protected final Token nextNN() throws ParseException {
		firstChar = NONE_C2C;
		Token w = next();
	...
	@Override
	protected final Token onSpecialToken(Token w) throws ParseException {
		CharSequence in = input;
		int i = index;
		CharList v = found; v.clear();
