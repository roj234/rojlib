...
public class YAMLParser extends Parser {
	...
	public CEntry parse(CharSequence text, @MagicConstant(flags = {ORDERED_MAP, LENIENT, NO_DUPLICATE_KEY}) int flag) throws ParseException {
		this.flag = flag;
		init(text);
	...
	final CEntry element(int flag) throws ParseException {
		...
		switch (w.type()) {
			...
			case Token.RFCDATE_DATE: return new CDate(w.asLong());
			case Token.RFCDATE_DATETIME, Token.RFCDATE_DATETIME_TZ: return new CTimestamp(w.asLong());
			case Token.LONG: return CEntry.valueOf(w.asLong());
			case TRUE, FALSE: {
			...
			case Token.EOF: return CNull.NULL;
		}
	}
	 ...
	 * c : x
	 */
	private CEntry yamlObject(Token w) throws ParseException {
		...
		while (true) {
			switch (w.type()) {
				...
				case join:
					...
					CEntry entry = anchors.get(w.text());
