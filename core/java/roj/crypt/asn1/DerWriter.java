...
public final class DerWriter {
	...
	public void begin(@MagicConstant(valuesFromClass = DerValue.class) int type) {
		T.put(type);
		L.add(0);
		V.add(null);
		stack.add(T.wIndex());
	...
	public void writeBool(boolean data) {write(DerValue.BOOLEAN, data ? TRUE : FALSE);}
	public void writeInt(BigInteger data) {write(DerValue.INTEGER, data.toByteArray());}
	public void writeBits(byte[] data, int bits) {
		assert (((data.length << 3) - bits) & ~7) == 0 : "invalid bit count "+bits+" for length["+data.length+"]";
	...
	public void writeText(@MagicConstant(intValues = {
			DerValue.UTCTime, DerValue.GeneralizedTime, DerValue.PrintableString, DerValue.IA5String, DerValue.UTF8_STRING
	}) int type, String text) {write(type, IOUtil.getSharedByteBuf().putAscii(text).toByteArray());}
