 ...
 */
public class FeedbackCipher extends RCipher {
	...
	protected final RCipher cip;
	protected ByteList vec, tmp;

	private byte type, padding;
	protected boolean decrypt;

	public FeedbackCipher(RCipher cip, @MagicConstant(intValues = {MODE_ECB,MODE_CBC,MODE_PCBC,MODE_CTS,MODE_OFB,MODE_CFB,MODE_CTR}) int mode) {
	...
	public FeedbackCipher(RCipher cip, String mode, String padding) throws NoSuchAlgorithmException, NoSuchPaddingException {
		if (!cip.isBareBlockCipher() || cip.engineGetBlockSize() == 0) throw new IllegalArgumentException("Not a block cipher");
