...
/**
 * @author Roj234
 * @since 2023/11/19 15:28
 */
public class Box {
	public static final Box DEFAULT = new Box("┌─┬┐\n│ ├┼┤└┴┘");
	// https://symbl.cc/cn/unicode/blocks/box-drawing/
	public static final Box BOLD = new Box("┏━┳┓\n┃ ┣╋┫┗┻┛");
	...
	public Box(String npr) {
		if (npr.length() != 13) throw new IllegalStateException("Illegal NPR");
		NPR = npr.toCharArray();
	}

	public void render(String[][] table) { render(Tty.getColumns(), table); }
	public void render(int _width, String[][] table) {
		boxOffset.clear();
		sb = IOUtil.getSharedCharBuf();
		...
		for (int i = 0; i < table.length; i++) {
			...
			for (int j = off; j < box.length; j++) {
				int w = Tty.getStringWidth(box[j]);
		...
		realWidth = table[0].length > 2 ? off-3 : off-2;
	...
	private void writeBox(String[] box, int i1) {
		...
		for (int i = i1; i < box.length; i++) {
			// -1来强制边距..算了,纯英文开比较好
			List<String> lines = Tty.splitByWidth(box[i], boxWidth[i-i1]);
		...
		for (List<String> line : totalLines) {
			...
			for (int i = 0; i < boxOffset.size(); i++) {
				...
				int width = Tty.getStringWidth(str);
				vOffset += width+2;
	...
	private void writeTop(String head) {
		int width = Tty.getStringWidth(head)+3;
