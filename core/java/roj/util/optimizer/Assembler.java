...
abstract class Assembler {
	...
	private static class win64 extends Assembler {
		...
		//     Java: rdx,  r8,  r9, rdi, rsi, rcx, stack (8-bit align)
		//   Native: rcx, rdx,  r8,  r9, stack (8-bit align)

		private static final int[] MOVE_INT_ARG = {
				0x89d1,  // mov  ecx, edx
				0b1000_100_1000100111_000010,  // mov  edx, r8d
				0b1000_101_1000100111_001000,  // mov  r8d, r9d
				0b1000_001_1000100111_111001,  // mov  r9d, edi
		};

		private static final int[] MOVE_LONG_ARG = {
				0b1001_000_1000100111_010001,  // mov  rcx, rdx
				0b1001_100_1000100111_000010,  // mov  rdx, r8
				0b1001_101_1000100111_001000,  // mov  r8, r9
				0b1001_001_1000100111_111001,  // mov  r9, rdi
		};

		private static final int MOVE_32 = 0b1000_000_1000100111_000000;
		private static final int MOVE_64 = 0b1001_000_1000100111_000000;
		// rdx, r8, r9, rdi << 16
		private static final int[] MOVE_64_SRC = {0b000,0b100,0b101,0b001};
		// rcx, rdx, r8, r9
		private static final int[] MOVE_64_DST = {0b010001, 0b000010, 0b001000, 0b111001};

		private static final int[] MOVE_OBJ_ARG = {
				0b1001_000_1000110101_001010,  // lea  rcx, [rdx+N]
				0b1001_001_1000110101_010000,  // lea  rdx, [r8+N]
				0b1001_101_1000110101_000001,  // lea  r8, [r9+N]
				0b1001_100_1000110101_001111,  // lea  r9, [rdi+N]
		};
		...
		static final class Mov {
			...
			@Override
			public String toString() {return "[rsp+"+read+"] + "+add+" => [rsp+"+write+"]";}
		}

		// dit not support float yet
		@Override void javaToCdecl(DynByteBuf buf, Class<?>[] types, Annotation[][] annotations) {
			int argno = types.length;

			int stackTop = 8;
			int endOffset = 0;
			var movs = new ArrayList<Mov>();
			int stackBottom = 40;
			...
			for (int i = 0; i < argno; i++) {
				...
				loadStack: {
					if (isFloat(type)) {
						if (floatArgIndex == 0) buf.putInt(0b11110010000011110001000011_011_001); // movsd xmm1 => xmm3
						else if (floatArgIndex == 1) buf.putInt(0b11110010000011110001000011_001_000); // movsd xmm0 => xmm1

						if (++floatArgIndex < 4) continue;
						break loadStack;
					} else if (intArgIndex < 4) {
						if (type.isPrimitive()) {
							//buf.putMedium((type == long.class ? MOVE_64 : MOVE_32) | (MOVE_64_SRC[intArgIndex] << 16) | (MOVE_64_DST[intArgIndex]));
							emit(buf, (type == long.class ? MOVE_LONG_ARG : MOVE_INT_ARG)[intArgIndex]);
						} else {
							emit(buf, MOVE_OBJ_ARG[intArgIndex]);
							buf.put(asByte(baseOffset(type, annotations[i])));
						}
					} else if (intArgIndex == 4) {
						int base;
						if (!type.isPrimitive() && baseOffset(type, annotations[i]) != 0) {
							base = 4;
							buf.preInsert(0, 9);
							buf.setInt(0, 0x4883C100 | baseOffset(type, annotations[i]));// add rcx, #imm8
						} else {
							base = 0;
							buf.preInsert(0, 5);
						}
						buf.setInt(base, 0b01001000_10001001_01_001_100_00100100).set(base+4, stackBottom);// [stack+16] = rcx
						ecxStackPos = base+4;
					} else if (intArgIndex == 5) {
						buf.set(ecxStackPos, buf.getByte(ecxStackPos) + 8);
						if (!type.isPrimitive()) buf.putInt(0x4883C600 | baseOffset(type, annotations[i]));// add rcx, #imm8
						buf.putInt(0b01001000_10001001_01_110_100_00100100).put(stackBottom);// [stack+8] = rsi
					} else {
						break loadStack;
					}
				...
				if (endOffset == 0) endOffset = (7 - argno + i) * 8;

				var item = new Mov(stackTop, endOffset);
			...
			var regmap = new IntMap<Mov>(movs.size());
			for (Mov mov : movs) {
				regmap.put(mov.write += stackSize, mov);
			}
			writeRegmap(buf, regmap);
		}

		private static boolean isFloat(Class<?> type) {return type == float.class || type == double.class;}
	...
	protected static int baseOffset(Class<?> type, Annotation[] annotations) {
		...
		if (type.isArray() && type.getComponentType().isPrimitive()) {
			return Unsafe.U.arrayBaseOffset(type);
		}
