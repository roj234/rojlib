...
public sealed class Bitmap {
	// Bitmap中每个单位为(1<<UNIT)字节
	private static final byte LONG_SHIFT = 6, UNIT = 3;
	public static final long UNIT_MASK = (1L << UNIT) - 1;
	...
	// 不要用非Public的方法
	public static Bitmap create(long capacity) {
	...
	Bitmap(int childId) { this.childId = (byte) childId; }
	...
	public CharList toString(CharList sb, int depth) {
		sb.append("[ depth = "+UNIT+" (unit)");
	...
	long malloc(long len) {
		int blocks = (int) ((len+UNIT_MASK) >>> UNIT);
		assert blocks <= 64;
		...
		bitmap |= flag;
		return (long) offset << UNIT;
	}
	boolean malloc(long off, long len) {
		int bitFrom = (int) (off >>> UNIT);
		int bitTo = (int) ((off+len+UNIT_MASK) >>> UNIT);
	...
	public long usedSpace() { return (long) Long.bitCount(bitmap) << UNIT; }
	public long freeSpace() { return (long) (64 - Long.bitCount(bitmap)) << UNIT; }
	public long totalSpace() { return 1L << (UNIT+LONG_SHIFT); }

	long headEmpty() { return (long) Long.numberOfTrailingZeros(bitmap) << UNIT; }
	long tailEmpty() { return (long) Long.numberOfLeadingZeros(bitmap) << UNIT; }

	boolean validate() { return true; }

	public static long align(long n) { return 0 == (n& UNIT_MASK) ? n : (n| UNIT_MASK) + 1; }
	...
	void getBlocks(long off, IntList list) {
		...
		if (state == 1) addLong(list, off + ((long)i << UNIT));
	}
	...
	private static sealed class Ext extends Bitmap {
		...
		private static final Bitmap[] EMPTY_BITMAPS = new Bitmap[0];
		private Bitmap[] child = EMPTY_BITMAPS;
		...
		final long malloc(final long size) {
			...
			if (block > 0) { // 大于1个完整的块
				...
				int offset = 0; // TODO lowestBits + increment in bulk
		...
		final boolean validate() {
			...
			for (int i = 0; i < childCount; i++) {
				Bitmap bitmap = child[i];
				try {
					bitmap.validate();
				} catch (AssertionError e) {
					AssertionError error = new AssertionError("child["+ bitmap.childId+"] validate() failed: ("+e.getMessage()+")"+this);
					error.setStackTrace(e.getStackTrace());
					throw error;
				}
				myFree += bitmap.freeSpace();
			}
		...
		public final void compress(MemoryMover m) {
			...
			Multisort.sort(0, data.size() / 4, (refA, posA, posB) -> {
		...
		private long success(long size, Bitmap p, long off) {
			free -= size;
		...
		private Bitmap goc(int blockId) {
			int i = get(blockId);
			...
			assert nextDepth >= UNIT : "invalid tree depth "+nextDepth;
			Bitmap p = nextDepth > UNIT ? new Ext(nextDepth, blockId) : new Bitmap(blockId);
