...
public class Multisort {
	 ...
	 * @implNote 这需要使用O(n * 2)的空间，目前只是简单的把结构拼在一起
	 */
	public static void sort(int fromIndex, int toIndex, MyComparator comparator, NativeArray... components) {
		if (toIndex - fromIndex <= 1) return;
		if (components.length == 0) throw new IllegalArgumentException("no array to sort");
		if (components.length == 1 && !components[0].isObjectArray()) {
			NativeArray op = components[0];
			sort(fromIndex, toIndex, comparator, op.ref, op.addr, op.compSize);
			return;
		}

		int compSize = 0;
		for (NativeArray op : components) compSize += op.compSize;
		...
		var temp = BufferPool.localPool().allocate(true, (int) (halfCap << 1), 0);
		long addr = temp.address();

		try {
			int offset = 0;
			for (NativeArray op : components) {
				op.copyTo(fromIndex, toIndex - fromIndex, null, addr+offset, compSize);
				offset += op.compSize;
			}

			timSort(fromIndex, toIndex, comparator, null, addr, addr+halfCap, compSize);
	...
	public static void sort(int leftIn, int rightEx, MyComparator cmp,
							Object ref, long off,
							int compSize) {
		var temp = BufferPool.localPool().allocate(true, compSize * (rightEx - leftIn), 0);
		try {
	...
	public static void timSort(int leftIn, int rightEx, MyComparator cmp,
		...
		int minRun = minRunLength(rightEx - leftIn);
