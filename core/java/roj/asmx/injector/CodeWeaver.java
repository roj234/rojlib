...
public class CodeWeaver implements Transformer {
	...
	private static final class IJPoint {
		...
		IJPoint(MethodNode method, Annotation map) {
			...
			boolean isAbstract = method.getAttribute("Code") == null;
			switch (map.type()) {
				case A_OVERWRITE -> {
					at = "LDC";
					extra = map;
		...
		do {
			for (BootstrapMethods.Item item : lambdaPending) {
				...
				for (int i = 0; i < args.size(); i++) {
					...
					CstRef ref = ((CstMethodHandle) c).getTarget();
			...
			for (MethodNode node : cv.copied) {
				Attribute cc = node.getAttribute("Code");
	...
	private static class Validator extends CodeVisitor {
		...
		public void visit(MethodNode node) {
			...
			Attribute code = node.getAttribute("Code");
			if (code != null) visit(data.cp, code.getRawData().slice());
		}
