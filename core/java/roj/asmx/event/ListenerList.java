...
public final class ListenerList {
			...
			for (int k = 0; k < instances.length; k++) {
				...
				for (int j = 0; j < size; j++) {
					...
					if (listener instanceof EventListenerImpl impl)
						U.compareAndSetReference(array, offset, listener, listener = impl.impl());
	...
	final void add(int priority, EventListener listener) {
		...
		if (instance == Collections.EMPTY_LIST) {
			...
			while (true) {
				if (U.compareAndSetReference(instances, offset, Collections.EMPTY_LIST, instance = new ArrayList<>())) break;
				if ((instance = (List<EventListener>) U.getReferenceVolatile(instances, offset)) != Collections.EMPTY_LIST) break;
			}
		}
