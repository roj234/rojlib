...
import static roj.asm.type.Generic.OR_TYPE;
...
public class TypeCast {
	...
	private Cast checkCast(IType from, IType to, int etype) {
		...
		switch (to.genericType()) {
			...
			case TYPE_PARAMETER_TYPE -> to = getTypeParamBound(to, typeParamsForTargetType != null ? typeParamsForTargetType : typeParams);
			case ANY_TYPE -> {
			...
			case CONCRETE_ASTERISK_TYPE -> to = ((Asterisk) to).getBound();
			case OR_TYPE -> {
				for (IType bound : ((Asterisk) to).getTraits()) {
		...
		switch (from.genericType()) {
			...
			case TYPE_PARAMETER_TYPE -> from = getTypeParamBound(from, typeParams);
			...
			case OR_TYPE -> {
				for (IType bound : ((Asterisk) from).getTraits()) {
		...
		if (asterisk != null) {
			List<IType> bounds = asterisk.getTraits();
	...
	static {
		WRAPPER.put(BOOLEAN, Type.klass("java/lang/Boolean"));
		WRAPPER.put(BYTE, Type.klass("java/lang/Byte"));
		WRAPPER.put(CHAR, Type.klass("java/lang/Character"));
		WRAPPER.put(SHORT, Type.klass("java/lang/Short"));
		WRAPPER.put(INT, Type.klass("java/lang/Integer"));
		WRAPPER.put(LONG, Type.klass("java/lang/Long"));
		WRAPPER.put(FLOAT, Type.klass("java/lang/Float"));
		WRAPPER.put(DOUBLE, Type.klass("java/lang/Double"));
		WRAPPER.put(VOID, Type.klass("java/lang/Void"));
	}
	public static int getWrappedPrimitive(IType self) { if (self instanceof Asterisk ax) self = ax.getBound(); return WRAPPER.getByValueOrDefault(self, 0); }
	@Nullable
