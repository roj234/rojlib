 ...
 * algorithm, 2005.
 * hxttp://homepage3.nifty.com/wpage/software/itssort.txt
 * </code></pre>
 * <p/>
 * This implementation is basically a translation of the C version given by Yuta Mori:
 * <tt>libdivsufsort-2.0.0, hxttp://code.google.com/p/libdivsufsort/</tt>
 * <p/>
 */
public final class DivSufSort {
	...
	private static final short[] SQQ_TABLE = {
	...
	private final int[] bucketA, bucketB;
	private final Stack ssStack, trStack, mergeStack;

	public DivSufSort() {
		this.bucketA = ArrayUtil.newUninitializedIntArray(256);
		this.bucketB = ArrayUtil.newUninitializedIntArray(65536);
	...
	private int buf(int index) {return Unsafe.U.getByte(buffer, buffer2 + index) & 0xFF;}

	private int init(byte[] b, int[] suffix, int length) {
		buffer = b;
		buffer2 = Unsafe.ARRAY_BYTE_BASE_OFFSET;
		sa = suffix;
		...
		Arrays.fill(bucketA, 0);
		Arrays.fill(bucketB, 0);

		return sortTypeBstar(bucketA, bucketB, length);
	}
	 ...
	 */
	public void computeSuffixArray(byte[] input, int[] sa, int length) {
		int m = init(input, sa, length);
	 ...
	 */
	public int computeBWT(byte[] input, byte[] output, int[] bwt,
						  int srcIdx, int dstIdx, int length,
						  int[] indexes, int idxCount) {
		int m = init(input, bwt, length);

		final int primaryIndex = constructBWT(bucketA, bucketB, length, m, indexes, idxCount);
		...
		for (int i = primaryIndex + 1; i < length; i++)
	...
	private int sortTypeBstar(int[] bucketA, int[] bucketB, final int n) {
		int m = n;
		int c0 = buf(n - 1);
		...
		if (m > 0) {
			...
			for (int i = n - 1, j = m; i >= 0; ) {
				...
				for (int c1 = c0; (i >= 0) && ((c0 = buf(i)) >= c1); i--) {
					c1 = c0;
				}
			...
			// Calculate the index of start/end point of each bucket.
			bucketB[bucketB.length - 1] = n; // end
			int k = m - 1;
			...
			for (c0 = 254; c0 >= 0; c0--) {
				...
				for (int c1 = 255; c1 > c0; c1--) {
					final int tt = i - bucketB[(c1 << 8) + c0];
	...
	private int ssCompare(int p1, int p2, int depth) {return ssCompare(sa[p1], sa[p1+1], p2, depth);}
	private int ssCompare(int pa, int pb, int p2, int depth) {
		...
		if (u1n - u1 > u2n - u2) {
			while (u2 < u2n && buf(u1) == buf(u2)) {
				u1++;
				u2++;
			}
		} else {
			while (u1 < u1n && buf(u1) == buf(u2)) {
				u1++;
				u2++;
			}
		}

		return u1 < u1n ? (u2 < u2n ? buf(u1) - buf(u2) : 1) : (u2 < u2n ? -1 : 0);
	}
	...
	private static int getIndex(int a) {return (a >= 0) ? a : ~a;}

	private void ssSwapMerge(int pa, int first, int middle, int last, int buf, int bufSize, int depth) {
		final int[] arr = this.sa;
		int check = 0;

		while (true) {
			HeyYouJavac_GiveMeGoto:
			if (last - middle <= bufSize) {
				if ((first < middle) && (middle < last))
					this.ssMergeBackward(pa, first, middle, last, buf, depth);

				//break HeyYouJavac_GiveMeGoto;
			} else {
				if (middle - first <= bufSize) {
					...
					if (((check & 4) != 0) && (ssCompare(pa + getIndex(arr[last - 1]), pa + arr[last], depth) == 0)) {
						arr[last] = ~arr[last];
					}

					if (!mergeStack.pop()) return;
				...
				if (m > 0) {
					...
					if (l - first <= last - r) {
						mergeStack.push(r, rm, last, (next & 3) | (check & 4), 0);
						middle = lm;
						last = l;
						...
						mergeStack.push(first, lm, l, (check & 3) | (next & 4), 0);
						first = r;
						middle = rm;
	...
	private void ssMultiKeyIntroSort(final int pa, int first, int last, int depth) {
		...
		while (true) {
			...
			if (limit < 0) {
				int v = buf(idx + this.sa[pa + this.sa[first]]);

				for (a = first + 1; a < last; a++) {
					if ((x = buf(idx + this.sa[pa + this.sa[a]])) != v) {
						if (a - first > 1)
							break;
				...
				if (buf(idx + this.sa[pa + this.sa[first]] - 1) < v)
			...
			final int v = buf(idx + this.sa[pa + this.sa[a]]);
			this.swapInSA(first, a);
			int b = first;

			// partition
			while (++b < last) {
				if ((x = buf(idx + this.sa[pa + this.sa[b]])) != v)
					break;
			}
			...
			while (--c > b) {
				if ((x = buf(idx + this.sa[pa + this.sa[c]])) != v)
					break;
			}
			...
			if (a <= d) {
				...
				a = first + (b - a);
				c = last - (d - c);
				b = (v <= buf(idx + this.sa[pa + this.sa[a]] - 1)) ? a : this.ssPartition(pa, a, c, depth);
	...
	private int ssMedian5(final int idx, int pa, int v1, int v2, int v3, int v4, int v5) {
		if (buf(idx + this.sa[pa + this.sa[v2]]) > buf(idx + this.sa[pa + this.sa[v3]])) {
			final int t = v2;
			v2 = v3;
			v3 = t;
		}

		if (buf(idx + this.sa[pa + this.sa[v4]]) > buf(idx + this.sa[pa + this.sa[v5]])) {
			final int t = v4;
			v4 = v5;
			v5 = t;
		}

		if (buf(idx + this.sa[pa + this.sa[v2]]) > buf(idx + this.sa[pa + this.sa[v4]])) {
			v4 = v2;
			final int t2 = v3;
			v3 = v5;
			v5 = t2;
		...
		if (buf(idx + this.sa[pa + this.sa[v1]]) > buf(idx + this.sa[pa + this.sa[v3]])) {
			final int t = v1;
			v1 = v3;
			v3 = t;
		}

		if (buf(idx + this.sa[pa + this.sa[v1]]) > buf(idx + this.sa[pa + this.sa[v4]])) {
			v4 = v1;
			v3 = v5;
		}

		return (buf(idx + this.sa[pa + this.sa[v3]]) > buf(idx + this.sa[pa + this.sa[v4]])) ? v4 : v3;
	}

	private int ssMedian3(int idx, int pa, int v1, int v2, int v3) {
		if (buf(idx + this.sa[pa + this.sa[v1]]) > buf(idx + this.sa[pa + this.sa[v2]])) {
			final int t = v1;
			v1 = v2;
			v2 = t;
		}
		...
		if (buf(idx + this.sa[pa + this.sa[v2]]) > buf(idx + this.sa[pa + this.sa[v3]]))
			return (buf(idx + this.sa[pa + this.sa[v1]]) > buf(idx + this.sa[pa + this.sa[v3]])) ? v1 : v3;
	...
	private void ssHeapSort(int idx, int pa, int saIdx, int size) {
		...
		if ((size & 1) == 0) {
			...
			if (buf(idx + this.sa[pa + this.sa[saIdx + (m >> 1)]]) < buf(idx + this.sa[pa + this.sa[saIdx + m]]))
	...
	private void ssFixDown(int idx, int pa, int saIdx, int i, int size) {
		...
		final int c = buf(idx + arr[pa + v]);
		int j = (i << 1) + 1;

		while (j < size) {
			int k = j;
			j++;
			int d = buf(idx + arr[pa + arr[saIdx + k]]);
			final int e = buf(idx + arr[pa + arr[saIdx + j]]);
	...
	private static int ssIlg(int n) {return ((n & 0xFF00) != 0) ? 8 + LOG_TABLE[(n >> 8) & 0xFF] : LOG_TABLE[n & 0xFF];}
	//endregion
	//region Tandem Repeat Sort
	...
	private void trIntroSort(int isa, int isad, int first, int last, TRBudget budget) {
		...
		final int[] arr = this.sa;
		final Stack trStack = this.trStack;
		...
		while (true) {
			if (limit < 0) {
				if (limit == -1) {
					...
					if (trStack.d == 0) {
						this.trCopy(isa, first, trStack.b, trStack.c, last, isad - isa);
					} else {
						trStack.clearLimit(trlink);
						this.trPartialCopy(isa, first, trStack.b, trStack.c, last, isad - isa);
					}
	...
	private static final class Stack {
		...
		Stack(int size) {array = new int[size * 5];}

		void clearLimit(int index) {
			if (index >= 0)
				array[index * 5 + 3] = -1;
		}
