...
public class QIncrementPak {
	 ...
	 */
	public static SevenZPacker openDualHeader(File baseFile) throws IOException {
		var source = getSource(baseFile);

		SevenZPacker out;
		if (source.length() > 0) {
			out = new SevenZFile(source).append();
			source.setLength(source.position());
		} else {
			out = new SevenZPacker(source);
			source.next();
		}
		return out;
	}
	 ...
	 * 关闭双头分卷压缩文件的写入器。
	 * <p>
	 * <b>重要提示：</b> 调用前必须关闭所有通过{@link SevenZPacker#newParallelWriter()}创建的并行写入器，
	 * 否则文件结构可能与预期不符（但文件仍可被读取）。
	 *
	 * @param qzfw 要关闭的QZFileWriter实例
	 * @throws IOException 当刷新或关闭操作失败时抛出
	 */
	public static void closeDualHeader(SevenZPacker qzfw) throws IOException {
		qzfw.flush();
		((CompositeSource) qzfw.getSource()).next();
		qzfw.close();
	}
	 ...
	 */
	public static SevenZPacker openSingleHeader(File baseFile) throws IOException {
		var source = getSource(baseFile);
		SevenZPacker out = source.length() > 0 ? new SevenZFile(source).append() : new SevenZPacker(source);
		source.next();
		return out;
	}
	 ...
	 * @throws IOException 当刷新或关闭操作失败时抛出
	 */
	public static void closeSingleHeader(SevenZPacker writer) throws IOException {
		writer.flush();

		CompositeSource s = (CompositeSource) writer.getSource();

		s.setSourceId(0);
		Source meta = s.getCurrentFragment();

		meta.seek(32);

		writer.flag = SevenZPacker.NO_TRIM_FILE;
		QZSH sh = (QZSH) writer;
	...
	@Weave(target = SevenZPacker.class)
	private static final class QZSHImpl implements QZSH {
		@Shadow(owner = "roj.archive.sevenz.SevenZWriter")
