...
import roj.util.function.Int2IntFunction;
 ...
 */
public class SevenZPacker extends SevenZWriter {
    private int compressHeaderMin = 1;
    private ExceptionalConsumer<XDataOutput, IOException> fileAttributes, archiveProperties;

    public SevenZPacker(String path) throws IOException {this(new FileSource(path));}
    public SevenZPacker(File file) throws IOException {this(new FileSource(file));}
    /**
     * 构建自定义数据源写入器（自动创建32字节头保留空间，并初始化为LZMA2 Level5编码器）
     * @param source 数据源对象
     * @throws IOException 当数据源初始化失败时抛出
     */
    public SevenZPacker(Source source) throws IOException {
        super(source);
        source.seek(32);
        setCodec(new LZMA2());
    }
     ...
     *   <li>如需禁用文件名加密，应在调用{@link #finish()}前设置此值为-1或通过{@link #setCodec(SevenZCodec...)}移除加密编码器</li>
    ...
    /**
     * 在kFilesInfo属性中设置任意的子属性，格式必须满足如下结构
     * ubyte ID;
     * VUInt length;
     * opaque[length] data;
     * 需要注意：
     * - ID必须在26-255内，7z容器格式并非设计为任意扩展
     * - 大部分实现都会忽略这些属性，包括我的
     * - 你需要带{@link SevenZFile#FLAG_LOAD_ATTRIBUTES}标志打开压缩包，之后使用{@link SevenZFile#getAttributes()}提取这些属性
     */
    public void setFileAttributes(ExceptionalConsumer<XDataOutput, IOException> fileAttributes) {
        this.fileAttributes = fileAttributes;
    }

    /**
     * 在kArchiveProperties属性中设置任意的子属性，格式必须满足如下结构
     * ubyte ID;
     * VUInt length;
     * opaque[length] data;
     * 需要注意：
     ...
     * - 你需要带{@link SevenZFile#FLAG_LOAD_ATTRIBUTES}标志打开压缩包，之后使用{@link SevenZFile#getAttributes()}提取这些属性
     */
    public void setArchiveProperties(ExceptionalConsumer<XDataOutput, IOException> archiveProperties) {
        this.archiveProperties = archiveProperties;
    }

    private List<ParallelPacker> parallelWriter;
     ...
     */
    public final SevenZWriter newParallelWriter() throws IOException {return newParallelWriter(new CacheSource());}
    /**
     * 创建自定义缓存的多线程并行写入器
     *
     * <p>注意事项：
     ...
     * @throws IllegalStateException 如果当前存在未关闭的WordBlock
     */
    public synchronized SevenZWriter newParallelWriter(Source cache) throws IOException {
        ...
        ParallelPacker pw = new ParallelPacker(cache);
    ...
    private class ParallelPacker extends SevenZWriter {
        ParallelPacker(Source source) { super(source, SevenZPacker.this); }

        @Override
        void finishWordBlock0() throws IOException {
            if (out == null) return;

            super.finishWordBlock0();
            SevenZPacker that = SevenZPacker.this;
            synchronized (that) {
     ...
     */
    public SevenZEntry removeEmptyFile(String name) {
		for (int i = 0; i < emptyFiles.size(); i++) {
			SevenZEntry entry = emptyFiles.get(i);
    ...
    public void removeEmptyFile(int i) {
        SevenZEntry entry = emptyFiles.remove(i);
        countFlag(entry.flag, -1);
    ...
    public void finish() throws IOException {
        ...
        try {
            if (compressHeaderMin == -1 ||
                ...
                blocks.add(metadata);
                finishWordBlock0();
                metadata.uSize = out.totalWritten();
        ...
        buf.putLong(SevenZFile.QZ_HEADER)
           .putIntLE(0)
           .putLongLE(hstart-32)
           .putLongLE(hend-hstart)
           .putIntLE(CRC32.finish(blockCrc32))
           .setIntLE(8, CRC32.crc32(buf.list, 12, 20));
    ...
    private void writeHeader(XDataOutput buf) throws IOException {
        ...
        if (archiveProperties != null) {
            buf.write(kArchiveProperties);
            archiveProperties.accept(buf);
            buf.write(kEnd);
        }
        ...
        if (files.size() > 0) {
            ...
            /*if ((flag&USE_ADDITIONAL_STREAMS) != 0) {
                buf.write(kAdditionalStreamsInfo);
    ...
    private void writeWordBlocks(XDataOutput buf) throws IOException {
        ...
        for (WordBlock b : blocks) {
            ...
            if (cc != null) {cc.writeCodecs(b, buf);continue;}

            buf.putVUInt(b.codecs.length);
            // always simple codec
            for (SevenZCodec c : b.codecs) {
                w.clear();
                c.writeOptions(w);
            ...
            // pipe
            for (int i = 0; i < b.codecs.length-1; i++)
        ...
        if (files.size() > blocks.size()) {
            ...
            if (flagSum[7] == files.size()) {
                ...
                for (int i = 0; i < blocks.size(); i++) {
                    ...
                    for (int k = b.fileCount; k > 0; k--) {
                        SevenZEntry entry = files.get(j++);
                        if ((entry.flag & SevenZEntry.CRC) != 0) {
                            set.add(extraCount);
    ...
    private void writeFilesInfo(XDataOutput buf) throws IOException {
        ...
        if (flagSum[0] > 0) {
            ...
            BitSet emptyFile = flagSum[1] > 0 ? new BitSet() : Helpers.maybeNull();
            BitSet anti = flagSum[2] > 0 ? new BitSet() : Helpers.maybeNull();
        ...
        int i;
        if ((i = flagSum[3]) > 0) writeTime(buf, kCTime, SevenZEntry.CT, i);
        if ((i = flagSum[4]) > 0) writeTime(buf, kATime, SevenZEntry.AT, i);
        if ((i = flagSum[5]) > 0) writeTime(buf, kMTime, SevenZEntry.MT, i);
        if ((i = flagSum[6]) > 0) writeWinAttributes(buf, i);

        if (fileAttributes != null) fileAttributes.accept(buf);
    ...
    private void writeFileNames(XDataOutput buf) throws IOException {
        ...
        buf.put(kName).putVUInt(len).put(0);
    ...
    private void writeWinAttributes(XDataOutput buf, int count) throws IOException {
        writeSparseHeader(buf, kWinAttributes, SevenZEntry.ATTR, count);
    ...
    private void writeTime(XDataOutput buf, int id, int flag, int count) throws IOException {
        ...
        long offset = SevenZEntryA.FIELD_OFFSET[id-kCTime];
        for (int i = 0; i < files.size(); i++) {
            SevenZEntry entry = files.get(i);
