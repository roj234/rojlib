 ...
 */
public class ConvolutionalCodec extends RCipher {
	...
	private Random rnd = CryptoFactory.WyRandom();

	public ConvolutionalCodec(int inBits, int outBits) {
		this.inBits = inBits;
	...
	private final BitStream ob = new BitStream();
	...
	public void encode(DynByteBuf data) {
		var ib = new BitStream(data);
		int history = buffer;

		for (int i = (ib.readableBits() + inBits - 1) / inBits; i > 0; i--) {
			history = (history << inBits) | ib.readBits(inBits);
	...
	public void decode(DynByteBuf data) {
		...
		for (int i = (ib.readableBits() + outBits - 1) / outBits; i > 0; i--) {
			var exceptingBuf = decodeSymbol(ib.readBits(outBits));
