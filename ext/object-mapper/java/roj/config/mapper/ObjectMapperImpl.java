...
final class ObjectMapperImpl extends ObjectMapper {
	...
	static {
		var cl = ObjectMapperImpl.class.getClassLoader();
		while (cl != null) {
			LOADER_CHAIN.add(cl);
	...
	private boolean strictTypeHandling;
	...
	static final class AsType {
		...
		final boolean isVisitor, needActualType;
		final boolean isGetterSetter;

		public AsType(String name, MethodNode writer, MethodNode reader) {
			this.name = name;
			this.instance = null;
			this.reader = reader;
			this.writer = writer;
			this.output = reader == null ? writer.returnType() : reader.parameters().get(0);
			this.isVisitor = false;
			this.needActualType = false;
			this.isGetterSetter = true;
		}

		public AsType(String name, MethodNode writer, MethodNode reader, Object o, boolean isVisitor, boolean needActualType) {
			this.name = name;
			this.instance = o;
			this.reader = reader;
			this.writer = writer;
			...
			this.needActualType = needActualType;
	...
	ObjectMapperImpl(int flag, ClassLoader cl) {
		super(flag);
		this.classLoader = cl;
	...
	@Override
	public ObjectMapper registerType(Class<?> type, SerializeSetting setting) {
	...
	@Override
	public ObjectMapper registerAdapter(Class<?> type, Object adapter, String serializeMethod, String deserializeMethod) {
		...
		if (ser == null) {
			...
			try {
				if ((ser = GENERATED.get(name)) == null) {
					ser = user(adapterType, data, w, r, isStatic);
	 ...
	 */
	@Override
	public ObjectMapper registerAdapter(Class<?> type, Object adapter) {
		if (type.isPrimitive() || type == String.class) throw new IllegalStateException("type不能是基本类型或字符串");
		...
		if (ser == null) {
			...
			try {
				if ((ser = GENERATED.get(name)) == null) {
					ser = user(adapterType, rw.data(), rw.writer(), rw.reader(), adapterType == adapter);
					GENERATED.put(name, ser);
				}
			} finally {
				lock.unlock();
	 ...
	 */
	@Override
	public ObjectMapper registerAdapter(String name, Class<?> type, Object adapter, String serializeMethod, String deserializeMethod) {
		var adapterType = adapter.getClass() == Class.class ? (Class<?>) adapter : adapter.getClass();
		var data = ClassNode.fromType(adapterType);
		if (data == null) throw new IllegalArgumentException("无法获取"+adapterType.getName()+"的类文件");
		...
		sp: {
			...
			if ((r.modifier&w.modifier&data.modifier&ACC_PUBLIC) == 0) throw new IllegalArgumentException("R/W方法和类本身都需要public");
	 ...
	 */
	@Override
	public ObjectMapper registerAdapter(String name, Class<?> type, Object adapter) {
		var adapterType = adapter.getClass() == Class.class ? (Class<?>) adapter : adapter.getClass();
		RW rw = autoRW(type, adapterType);
		synchronized (asTypes) {
	...
	@Override
	public ObjectReader<?> reader(Class<?> type, IType generic, boolean pooled) {
		TypeAdapter t = generic != null ? get(generic) : make(null, type, null, false);
		return pooled ? new ReentrantObjectReader(t) : new MappingContext(t);
	}

	@Override
	public ObjectWriter<?> writer(Class<?> type, IType generic, boolean pooled) {
		TypeAdapter t = generic != null ? get(generic) : make(null, type, null, false);
		return pooled ? new ReentrantObjectWriter(t) : t;
	}
	...
	final TypeAdapter get(IType generic) {
		...
		if (!(generic instanceof ParameterizedType g) || (flag & PREFER_DYNAMIC_0) != 0) {
			return make(null, klass, null, false);
		} else {
			return make(generic.toDesc(), klass, g, false);
		}
	}
	//ObjAny
	...
	private TypeAdapter make(String name, @NotNull Class<?> type, ParameterizedType parameterizedType, boolean mustExact) {
		...
		if (parameterizedType != null) {
			...
			if (ser != null) return ser.transform(this, type, parameterizedType.typeParameters);
		}
		...
		if ((flag & (CHECK_INTERFACE|CHECK_PARENT)) != 0) {
			...
			List<IType> typeParameters = parameterizedType == null ? null : parameterizedType.typeParameters;
			try {
				var genericType = parameterizedType == null ? type : RuntimeTypeInference.fromAsmType(parameterizedType, classLoader);
				var inferTypeParameters = RuntimeTypeInference.inferGeneric(genericType, c);
				if (!inferTypeParameters.isEmpty()) {
					typeParameters = new ArrayList<>(inferTypeParameters.size());
					for (var arg : inferTypeParameters) {
						typeParameters.add(RuntimeTypeInference.toAsmType(arg));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
			...
			ser = ser.transform(this, type, typeParameters);
	...
	final TypeAdapter gsa(Class<?> type, String generic) {
		...
		if ((flag & PREFER_DYNAMIC_0) != 0) {
			...
			parameterizedType = Signature.parse(generic, Signature.FIELD).values.get(0) instanceof ParameterizedType g ? g : null;
		}

		TypeAdapter make = make(generic, type, parameterizedType, false);
		if ((this.flag&OBJECT_POOL) != 0 && make != dynamicRoot) make = new PooledAdapter(make);
		return make;
	}
	// Get parent adapter
	...
	final TypeAdapter gpa(Class<?> type) {
		TypeAdapter ser = make(type, type.getName().replace('.', '/'), SERIALIZE_PARENT);
	 ...
	 * 包装用户提供的序列化器
 	 */
	private TypeAdapter user(Class<?> o, ClassNode data, MethodNode writer, MethodNode reader, boolean isStatic) {
	...
	// region 对象序列化器
	private static final byte DIRECT_IF_OVERRIDE = MAGIC_ADAPTER ? INVOKESPECIAL : INVOKEVIRTUAL;
	/**
	 * 对象序列化器
	 */
	private TypeAdapter klass(Class<?> o, int flag, SerializeSetting fp) {
		...
		if (data == null) throw new IllegalArgumentException("无法找到"+o.getName()+"的字节码");
		...
		if ((flag & SERIALIZE_PARENT) != 0 && !"java/lang/Object".equals(data.parent()) && data.parent() != null) {
			...
			copy.invoke(DIRECT_IF_OVERRIDE, "roj/config/mapper/ObjectMapperImpl", "gpa", "(Ljava/lang/Class;)Lroj/config/mapper/TypeAdapter;");
			...
			optional = parentSerInst.plusOptional(optionalEx, 0);
		...
		int writeObject = data.getMethod("writeObject", "(Lroj/config/ValueEmitter;)V");
		if (writeObject >= 0) {
			write.visitSize(2, 3);
			write.insn(ALOAD_1);
			write.invokeV(data.name(), "writeObject", "(Lroj/config/ValueEmitter;)V");
			write.insn(RETURN);
			write.finish();
			write = null;
		} else {
		...
		cw.visitSize(3,3);
		cw.insn(ALOAD_1);
		// NoSuchMethodError on deserialize
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			AsType as = null;
			String comment = null;
			boolean getterChangedType = false;
				...
				switch (anno.type()) {
					...
					case "roj/config/mapper/Via" -> {
						...
						if (sid != null) {
							...
							for (int size = methods.size(); id < size; id++) {
								var ms = methods.get(id);
								if (sid.equals(ms.name()) && ms.rawDesc().startsWith("()")) break;
							}
							if (id == methods.size()) throw new IllegalArgumentException("无法找到get方法"+anno);

							get = data.methods.get(id);
							getterChangedType |= !get.returnType().equals(f.fieldType());
							unsafe &= 5;
						}
						...
						if (sid != null) {
							...
							for (int size = methods.size(); id < size; id++) {
								var ms = methods.get(id);
								if (sid.equals(ms.name()) && ms.rawDesc().endsWith(")V") && ms.parameters().size() == 1) break;
							}
							if (id == methods.size()) throw new IllegalArgumentException("无法找到get方法"+anno);

							set = data.methods.get(id);
							getterChangedType |= !set.parameters().get(0).equals(f.fieldType());
							unsafe &= 2;
						}

						if (getterChangedType && (get == null || set == null))
							throw new IllegalArgumentException("转换了真实类型时Getter/Setter必须全部提供");
					}
					case "roj/config/mapper/Optional" -> {
			...
			if (as != null) {
				if (getterChangedType) throw new IllegalArgumentException("Getter/Setter转换了真实类型时，无法使用As");
				type = as.output;
			}
			if (get != null) type = get.returnType();
			asmStruct(fieldId, data, f, type, name, get, set, writeMode, noRead, as, (unsafe&4) != 0 ? o : null, comment, nullValuePattern);
		...
		if (optional != 0 || optionalEx != null) {
			...
			// region optional
			cw = c.newMethod(ACC_PUBLIC|ACC_FINAL, "plusOptional", "(Lroj/collect/BitSet;I)I");
			cw.visitSize(2,3);
			if (parentSer >= 0) {
	...
	private void asmStruct(final int fieldId, final ClassNode data, final FieldNode fn, final Type type,
		...
		if (as != null && as.instance != null) {
			...
			if (asId < 0) {
				...
				copy.invoke(DIRECT_IF_OVERRIDE, "roj/config/mapper/ObjectMapperImpl", "gas", "(Ljava/lang/String;)Ljava/lang/Object;");
		...
		if (as != null) {
			cw.visitSizeMax(3+type.length(), 0);
			if (as.needActualType) cw.ldc(fn.rawDesc());
			cw.invoke(as.instance == null ? INVOKESTATIC : INVOKEVIRTUAL, as.reader);
		}

		if (set == null) {
			if (unsafePut != null) {
				VarHandleRewriter.ensureClassInitialized();

				//noinspection MagicConstant
				Type fType = Type.primitive(actualType);
				cw.visitSizeMax(4+fType.length(), 0);

				cw.varStore(fType, 2);
				try {
					cw.ldc(Unsafe.objectFieldOffset(unsafePut, fn.name(), fn.fieldType().toClass(unsafePut.getClassLoader())));
				} catch (ClassNotFoundException e) {
					throw new TypeNotPresentException(fn.fieldType().toDesc(), e);
				}
				cw.insn(ALOAD_3);
				cw.varLoad(fType, 2);
				String capitalized = fType.capitalized();
				cw.invokeS(VarHandleRewriter.PROXY_NAME, "put"+(capitalized.equals("Object")?"Reference":capitalized), "(JLjava/lang/Object;"+(fType.isPrimitive() ? fType.toDesc() : "Ljava/lang/Object;")+")V");
			} else {
		...
		Label skip = null;
		cw = write;
		if (cw != null && !"SKIP".equals(writeMode)) {
			// 如果可选并且不是基本类型
			_ActualTypeIsClass:
					...
					case Type.CLASS -> {
						...
						if (writeMode.equals("NON_BLANK")) {
							...
							if (actualTypeName.startsWith("[")) {
								...
								Class<?> type111;
								try {
									type111 = type.toClass(classLoader);
								} catch (ClassNotFoundException e) {
									throw OperationDone.NEVER;
								}

								if (String.class.isAssignableFrom(type111)) {
									cw.vars(ALOAD, 4);
									cw.invokeV("java/lang/String", "isEmpty", "()Z");
									cw.jump(IFNE, skip);
								} else if (CharSequence.class.isAssignableFrom(type111)) {
									cw.vars(ALOAD, 4);
									cw.invokeItf("java/lang/CharSequence", "length", "()I");
									cw.jump(IFEQ, skip);
								} else if (Map.class.isAssignableFrom(type111)) {
									cw.vars(ALOAD, 4);
									cw.invokeItf("java/util/Map", "isEmpty", "()Z");
									cw.jump(IFNE, skip);
								} else if (Collection.class.isAssignableFrom(type111)) {
									cw.vars(ALOAD, 4);
									cw.invokeItf("java/util/Collection", "isEmpty", "()Z");
									cw.jump(IFNE, skip);
								} else if (java.util.Optional.class.isAssignableFrom(type111)) {
									cw.vars(ALOAD, 4);
		...
		if (actualType == Type.CLASS && !"java/lang/String".equals(type.getActualClass())) {
			...
			cw = write;
			if (cw == null || "SKIP".equals(writeMode)) return;
			cw.insn(ALOAD_0);
			cw.field(GETFIELD, c, id);
			cw.insn(ALOAD_1);
			if (as != null && as.instance != null) {
			...
			if (as != null) cw.invoke(as.instance == null ? INVOKESTATIC : INVOKEVIRTUAL, as.writer);
			...
			if (cw == null || "SKIP".equals(writeMode)) return;
			if (as != null) {
				if (!as.isVisitor) cw.insn(ALOAD_1);
				if (as.instance != null) {
					cw.insn(ALOAD_0);
					cw.field(GETFIELD, c, asId);
				}
			...
			if (as != null) {
				if (as.isVisitor) cw.insn(ALOAD_1);
				cw.invoke(as.instance == null ? INVOKESTATIC : INVOKEVIRTUAL, as.writer);
				if (as.isVisitor) break block;
			}
	...
	private String typeParamToRaw(Signature fieldSig, Signature typeSig) {
		...
		List<IType> values = fieldSig.values;
		values.set(0, RuntimeTypeInference.substituteTypeVariables(values.get(0), mapper));
		return fieldSig.toDesc();
	}
	...
	private Tmp2 createReadMethod(ClassNode data, int methodType) {
		...
		if (parentExist.remove(methodType)) {
			...
			if (strictTypeHandling) {
				...
				cw.insn(ALOAD_0);
				cw.insn(ALOAD_1);
				cw.vars(Type.getMethodTypes(desc).get(1).getOpcode(ILOAD), 2);
				cw.invoke(INVOKESPECIAL, "roj/config/mapper/TypeAdapter", "read", desc);
			}
			cw.insn(RETURN);
		}
	...
	private int ser(String type, String generic) {
		...
		if (!type.equals(currentObject)) {
			...
			if (generic == null || generic.equals("java/lang/Object")) {
				copy.invoke(INVOKEVIRTUAL, "roj/config/mapper/ObjectMapperImpl", "get", "(Ljava/lang/Class;)Lroj/config/mapper/TypeAdapter;");
			} else {
				copy.ldc(new CstString(generic));
				copy.invoke(INVOKEVIRTUAL, "roj/config/mapper/ObjectMapperImpl", "gsa", "(Ljava/lang/Class;Ljava/lang/String;)Lroj/config/mapper/TypeAdapter;");
			}
		} else {
			copy.insn(ALOAD_0);
		}
	...
	private void begin(String ua) {
		...
		copy = c.newMethod(ACC_PUBLIC|ACC_FINAL, "init2", "(Lroj/config/mapper/ObjectMapperImpl;Ljava/lang/Object;)V");
		copy.visitSize(4, 3);
	}
	private TypeAdapter build(Class<?> type) {
		ClassNode node = c;
		...
		if (serializerId.isEmpty()) {
			node.methods.remove(node.getMethod("init2"));
		} else {
			copy.insn(RETURN);
			copy.finish();
		}
		...
		if (type.getClassLoader() == null || LOADER_CHAIN.contains(type.getClassLoader())) {
			type = GA.class;
		}
		return (TypeAdapter) Reflection.createInstance(type, node);
	}
	// endregion
}