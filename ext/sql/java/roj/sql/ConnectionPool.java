...
/**
 * @author Roj234
 * @since 2023/5/17 9:20
 */
public class ConnectionPool implements DataSource {
	final DataSource connector;
	...
	public ConnectionPool(DataSource connector, int pooledConnections) {
	...
	@Override
	public ConnectionPool pooled(int connections) {throw new UnsupportedOperationException("already pooled");}
	...
	private Connection wrap(Connection connection) {
		if (proxy == null) {
			...
			synchronized (ConnectionPool.class) {
				...
				data.name("roj/sql/ConnectionPool$PooledConnection");
				...
				TransformUtil.proxyClass(data, new Class<?>[]{Connection.class}, (m, c) -> {
					if (m.getName().equals("close")) {
						...
						c.insn(Opcodes.ALOAD_0);
						c.invokeV("roj/sql/ConnectionPool", "_reserve", "(Ljava/sql/Connection;)V");
						return true;
						...
						c.insn(Opcodes.ALOAD_0);
						c.field(Opcodes.GETFIELD, data, closeHandler);
						c.invokeV("roj/sql/ConnectionPool", "_isClosed", "()Z");
						return true;
					}
					return false;
				}, closeHandler);

				proxy = Helpers.cast(Reflection.createInstance(ConnectionPool.class, data));
			}
		}
