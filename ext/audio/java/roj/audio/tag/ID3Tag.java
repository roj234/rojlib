...
public class ID3Tag implements AudioMetadata {
	...
	private Multimap<String, Object> attributes = null;
	...
	public Multimap<String, Object> getAttributes() {
		if (attributes == null) attributes = new Multimap<>();
		return attributes;
	}

	public void setAttribute(String name, Object value) {getAttributes().put(name, value);}
	@Override public <T> @Nullable T getAttribute(TypedKey<T> key) {return attributes == null ? null : key.cast(attributes.get(key.name));}
		...
		try {
			...
			switch (tag) {
				...
				case "USLT" -> {    //USLT:  歌词
					...
					String language = in.readAscii(3);
					...
					System.out.println("[ID3]  Land="+language);
