...
public class FFT {
     ...
     */
    public static Vec2d[] fft(@MayMutate Vec2d[] data) {fft(data, - Math.PI);return data;}
     ...
     * @see #fft(Vec2d[])
     */
    public static Vec2d[] ifft(@MayMutate Vec2d[] data) {
        // 使用反向旋转因子内联共轭操作
        fft(data, Math.PI);
     ...
     */
    public static void fft(@MayMutate Vec2d[] data, double angle) {
        int n = data.length;
     ...
     * @see #ifft(Vec2d[])
     */
    public static void scale(@MayMutate Vec2d[] data, double factor) {
        for (Vec2d v : data) {
            v.x *= factor;
            v.y *= factor;
        }
