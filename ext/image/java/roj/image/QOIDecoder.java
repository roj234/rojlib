...
public class QOIDecoder implements ImageDecoder {
	...
	@Override
	public boolean init(XDataInput in) throws IOException {
	...
	@Override
	public BufferedImage decodeImage(XDataInput in) throws IOException {
		ColorModel cm = getColorModel();
		WritableRaster raster = cm.createCompatibleWritableRaster(width, height);

		int[] argb = ((DataBufferInt) raster.getDataBuffer()).getData();
		decodeImage(in, new IntConsumer() {
			int i = 0;
			public void accept(int value) { argb[i++] = value; }
		});
	...
	@Override
	public void decodeImage(XDataInput in, IntConsumer argb) throws IOException {
		int i = 0;
				...
				switch (b1 >>> 6) {
					...
					case 3 -> {
						if (b1 >= 0b11111110) { // RGB
							...
							if (i+b1 >= pixels) throw new CorruptedInputException("RLE out of range");
