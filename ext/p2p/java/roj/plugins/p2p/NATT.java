...
public final class NATT implements Closeable, ChannelHandler, Consumer<MyChannel> {
		...
		switch (result) {
			...
			case 0:
				if (remote == null) {
					...
					String addr = TUI.input("请输入对方的连接码: ");
					natt.connect(addr);
	...
	public static Boolean checkTCPPort(int port) {
		try {
			ByteList data = HttpRequest.builder().uri("http://portcheck.transmissionbt.com/"+port).execute(10000).bytes();
