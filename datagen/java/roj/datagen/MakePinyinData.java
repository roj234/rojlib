...
public class MakePinyinData {
	public void run(File input, File output) throws Exception {
		process(new File(input, "pinyin-data-master.txt"), new File(input, "phrase-pinyin-data-master.txt"), new File(output, "roj/plugins/pinyin/JPinyin.lzma"));
	}

	private static void process(File pinyinData, File phrasePinyinData, File output) throws Exception {
		TrieTree<String[]> pinyinMap = new TrieTree<>();
		try (TextReader in = TextReader.auto(pinyinData)) {
		...
		// 让压缩率再高一点吧~
		try (OutputStream out = options.getOutputStream(new FileOutputStream(output))) {
