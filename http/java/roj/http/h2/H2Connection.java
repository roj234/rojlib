...
public class H2Connection implements ChannelHandler {
	...
	private void frame(ChannelCtx ctx, DynByteBuf buf) throws IOException {
		...
		if (type > FRAME_BLOCKED) {error(H2Exception.ERROR_PROTOCOL, "Unknown.Frame.Type");return;}

		int flag = buf.readUnsignedByte();
		if ((flag & ~KNOWN_FLAGS[type]) != 0) {error(H2Exception.ERROR_PROTOCOL, "Unknown.Frame.Flag");return;}

		int id = buf.readInt();
		if (id < 0) {error(H2Exception.ERROR_PROTOCOL, "StreamId");return;}
		if (id > nextStreamId) nextStreamId = id+1;

		H2Stream c;
		switch (type) {
			case FRAME_SETTINGS -> {
				if (id != 0) error(H2Exception.ERROR_PROTOCOL, "StreamId");
			...
			case FRAME_HEADER -> {
				...
				if (c == null) {
					if (id != 0 && isServer()) {
						...
						if (streams.size() >= localSetting.max_streams && localSetting.max_streams != -1) {streamError(id, H2Exception.ERROR_REFUSED);return;}
						if (id < nextStreamId-1) {error(H2Exception.ERROR_PROTOCOL, "StreamId");return;}
						c = newStream(id);
					} else {invalidStream(id);return;}
				}
				if (hpackLock != 0 && hpackLock != id) {error(H2Exception.ERROR_PROTOCOL, "Continuous.Expected");return;}
				...
				if (err != null) error(H2Exception.ERROR_PROTOCOL, err);
				else {
			...
			case FRAME_CONTINUATION -> {
				if (hpackLock != id) {error(H2Exception.ERROR_PROTOCOL, "Continuous.Unexpected");return;}
				c = streams.get(id);
			...
			case FRAME_DATA -> {
				...
				if ((receiveWindow -= len) < 0) error(H2Exception.ERROR_FLOW_CONTROL, "Data.Window");
				else if ((c.receiveWindow -= len) < 0) streamError(c.id, H2Exception.ERROR_FLOW_CONTROL);
				else {
					...
					if (err != null) streamError(id, H2Exception.ERROR_PROTOCOL);
					else if ((flag & FLAG_END) != 0) dataEnd(c);
				}
			}
			case FRAME_PUSH_PROMISE -> {
				if (!localSetting.push_enable || isServer()) {error(H2Exception.ERROR_PROTOCOL, "Push.Enable");return;}
				c = streams.get(id);
				if (c == null) {invalidStream(id);return;}
				if (hpackLock != 0 && hpackLock != id) {error(H2Exception.ERROR_PROTOCOL, "Continuous.Expected");return;}
				...
				id = buf.readInt();
				if ((id&1) == (nextStreamId&1)) {error(H2Exception.ERROR_PROTOCOL,"StreamId");return;}
			...
			case FRAME_PING -> {
				...
				else if (id != 0) error(H2Exception.ERROR_PROTOCOL, "StreamId");
				...
				} else if (ping == null || buf.readLong() != (ping.sendTime ^ ping.nonce)) error(H2Exception.ERROR_PROTOCOL, "Ping.Ack");
				else {
			...
			case FRAME_GOAWAY -> {
				if (buf.readableBytes() < 8) sizeError();
					...
					if (errno != H2Exception.ERROR_OK) LOGGER.warn("远程发送的Goaway({}): {}", errno, reason);
			...
			case FRAME_WINDOW_UPDATE -> {
				...
				int win = buf.readInt();
				if (win <= 0) {
					if (id == 0) error(H2Exception.ERROR_PROTOCOL, "WindowSize");
					else streamError(id, H2Exception.ERROR_PROTOCOL);
					return;
				}

				if (id == 0) {
					if ((win += sendWindow) < 0) error(H2Exception.ERROR_FLOW_CONTROL, "WindowSize");
					sendWindow = win;
				} else {
					c = streams.get(id);
					...
					if ((win += sendWindow) < 0) streamError(id, H2Exception.ERROR_FLOW_CONTROL);
					c.sendWindow = win;
	...
	private void headerEnd(H2Stream c) throws IOException {
		...
		if (err != null) streamError(c.id, H2Exception.ERROR_PROTOCOL);
	}
		...
		if ((flag & FLAG_SERVER) != 0) {
			if (size < 0) {
				if (System.currentTimeMillis() - dosTime < 10 && ++dosCount2 > 7) {
					error(H2Exception.ERROR_CALM_DOWN, info);
				} else {
					dosCount2 = 0;
				}
	...
	private void unpad(DynByteBuf buf) {
		...
		if (len < 0) error(H2Exception.ERROR_PROTOCOL, "Padding");
		else buf.wIndex(len);
	}
	...
	private void checkGoaway() throws IOException {
		...
		if (streams.isEmpty()) {
			if ((flag&FLAG_GOAWAY_SENT) == 0) goaway(H2Exception.ERROR_OK, "");
			try {
	...
	public void exceptionCaught(ChannelCtx ctx, Throwable ex) throws Exception {
		...
		error(H2Exception.ERROR_INTERNAL, ex.toString());
	...
	protected void streamErrorCaught(int id, Throwable ex) {
		...
		streamError(id, H2Exception.ERROR_INTERNAL);
	}
	// connection error
	private void sizeError() {error(H2Exception.ERROR_FRAME_SIZE, null);}
	public void error(int errno, String reason) {
		if (errno != H2Exception.ERROR_INTERNAL) LOGGER.debug("协议错误({}): {}", errno, reason);
	...
	private void invalidStream(int id) {
		if (id == 0) error(H2Exception.ERROR_PROTOCOL, "StreamId");
		else streamError(id, H2Exception.ERROR_STREAM_CLOSED);
	}
	public void streamError(int id, int errno) {
		if (errno != H2Exception.ERROR_OK) LOGGER.debug("流[{}/#{}]关闭: {}", this, id, errno);
