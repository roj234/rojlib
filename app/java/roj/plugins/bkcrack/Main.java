...
public class Main extends Plugin {
	...
	private static void tryGetPlain(Executor pool, File f, int off) throws IOException {
	...
	private static void tryFindPass(String entryName, String[] args, File file, int argOff, boolean stopOnFirstKey) throws IOException {
		...
		if(keys.isEmpty()) {
			...
			Cipher selection;
			if (keys.size() > 1) {
				selection = TUI.radio(TUI.text("选择一个密钥"), keys, o -> new Completion(String.valueOf(o)));
			} else {
				selection = keys.get(0);
			}

			IOUtil.copyFile(new File(file.getAbsolutePath()+".key."+keys.indexOf(selection)), new File(file.getAbsolutePath()+".key"));
