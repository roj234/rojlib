...
public class MCTrim extends Plugin {
	...
	protected void onEnable() throws Exception {
		registerCommand(literal("mctrim").then(argument("assets", Argument.folder()).then(argument("libraries", Argument.folder()).executes(ctx -> {
			...
			while (true) {
				File file = c.readSync(Argument.folder());
				if (file == null) break;
				versions.add(file);
			}
		...
		while (true) {
			...
			switch (selection) {
				...
				case 'L', 'l':
					...
					break;
				case 'S', 's':
					File file = new Shell("保存到 > ").readSync(Argument.fileOptional(true));
