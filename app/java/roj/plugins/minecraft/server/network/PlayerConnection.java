...
public class PlayerConnection implements ChannelHandler/*, ViaCommandSender*/ {
	...
	private TimerTask keepaliveTask, tickTask;
	...
	public void handlerAdded(ChannelCtx ctx) {
		...
		try {
			...
			Timer timer = MinecraftServer.INSTANCE.getScheduler();
			keepaliveTask = timer.loop(() -> {
				if (sendKeepAilve) {
					...
					try {
						...
						try {
							ctx.channel().exceptionCaught("Tick", e);
						} catch (Exception ex) {
							ex.printStackTrace();
						}
					}
			...
			tickTask = timer.loop(this::tick, 50);
