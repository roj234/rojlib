...
public class CardSleep extends JFrame {
	...
	private static long prevLog;
	private static boolean prevRepaint;
	...
	public CardSleep() {
		...
		uiCardList.addMouseListener(new DoubleClickHelper(uiCardList, 500, list -> {
			...
			query(line -> {
				...
				if (coreFreq1 > entry.value) entry.value = coreFreq1;
	...
	private boolean setClock(boolean isGraphicClock, int frequency, boolean isLocked) {
		...
		if (isGraphicClock) {
			SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(this, "该显卡不支持或没有权限调节核心频率"));
			uiST_Core.setSelected(false);
			uiST_Core.setEnabled(false);
		} else {
			SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(this, "该显卡不支持或没有权限调节显存频率"));
			uiST_Memory.setSelected(false);
