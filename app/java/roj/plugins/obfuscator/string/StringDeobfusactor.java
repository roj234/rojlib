	...
	protected void decryptString(List<Context> arr) {
		...
		for (int i = 0; i < arr.size(); i++) {
			...
			for (int j = 0; j < methods.size(); j++) {
				...
				if ((mn.modifier()&(Opcodes.ACC_STATIC | Opcodes.ACC_PRIVATE)) == Opcodes.ACC_STATIC &&
					...
					var code0 = mn.getAttribute(data.cp, "Code");
					try {
						filter.visit(data.cp, code0.getRawData().slice());
		...
		for (int i = 0; i < arr.size(); i++) {
			...
			for (int j = 0; j < methods.size(); j++) {
				...
				var code0 = m.getAttribute(data.cp, "Code");
				if (code0 == null) continue;

				Code code = new Code(code0.getRawData().slice(), data.cp, m);
				try {
