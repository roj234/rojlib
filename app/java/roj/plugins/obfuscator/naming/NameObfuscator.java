...
public class NameObfuscator implements ObfuscateTask {
	...
	public Random rand = CryptoFactory.WyRandom();
	...
	public void forEach(List<Context> arr, Random rand, Executor executor) {
		...
		Profiler.startSection("initResolver");
		...
		Profiler.endStartSection("makeMap");
	...
	private void generateObfuscationMap(Resolver resolver, Context c) {
		...
		String to = clazz == null || (exclusion&EX_CLASS)!=0 ? from : clazz.obfClass(from, m.getClassMap().inverse().keySet(), rand);
