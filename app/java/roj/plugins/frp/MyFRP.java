...
public class MyFRP implements ChannelHandler {
	...
	private void start(File file) throws Exception {
		...
		InetSocketAddress address = Net.parseConnectAddress(yml.getString("server"));
		...
		/*if (yml.getBool("room_owner")) {
			...
			if (whitelist.size() > 0) {
				var userWhiteList = h.whitelist = new HashMap<>(whitelist.size());
	...
	private DefaultListModel<Obsoleted> model = new DefaultListModel<>();
	...
	private static String res(String name) throws IOException {
		...
		if (v == null) res.put(name, v = IOUtil.getTextResourceIL("META-INF/html/"+name));
		return v;
	}
