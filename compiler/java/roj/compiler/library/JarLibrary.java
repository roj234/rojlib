...
public class JarLibrary implements Library {
	...
	public ClassNode get(CharSequence name) {
		try (var in = jar.getInputStream(name.toString().concat(".class"))) {
	...
	@Override public InputStream getResource(CharSequence name) throws IOException {return jar.getInputStream(name.toString());}
