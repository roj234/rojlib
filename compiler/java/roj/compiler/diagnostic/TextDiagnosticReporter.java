	...
	public Boolean apply(Diagnostic diag) {
		...
		if (diag.getLine() != null) {
			...
			if (diag.getLength() > 0) {
				...
				for (int i = 0; i < diag.getColumnNumber(); i++) {
					realWidth += Tty.getCharWidth(line.charAt(i));
				}
				sb.padEnd(' ', realWidth).append('^');
			}
		}
		if (DebugSetting.ShowFullDiagnostic) sb.append('\n').append(diag.getCode());
		...
		for (int i = 2; i < trace.length; i++) {
			...
			if (!name.equals("report") && !name.equals("fireDiagnostic")
					&& !name.equals("castTo") && !name.equals("writeCast")
					&& !name.equals("endCodeBlock") && !name.endsWith("OrReport")
					&& !name.equals("reportNoSuchType")
					&& !name.equals("checkAccessModifier") && !name.startsWith("canAccess")
					&& !name.equals("findField") && !name.equals("findMethod")) {
				System.err.println("    at "+trace[i]);
				if (!DebugSetting.ShowFullDiagnostic) break;
			}
		}
