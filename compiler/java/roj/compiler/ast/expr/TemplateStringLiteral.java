	...
	public Expr resolve(CompileContext ctx) throws ResolveException {
		...
		switch (type) {
			case "f" -> {
				...
				var tag = template;
				var prevIndex = ctx.tokenizer.prevIndex;
				int prevI = 0;
				while (true) {
				...
				ctx.tokenizer.setText(ctx.file.getCode(), prevIndex);
				return concat;
			}
			case "b" -> {
			...
			default -> {
				Expr override = ctx.getOperatorOverride(processor, type, Token.STRING);
