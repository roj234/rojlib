		...
		if (exist && cw.method.owner().equals(this.owner.name())) {
			...
			if (readAccessor < 0) {
				...
				if ((fn.modifier & Opcodes.ACC_STATIC) != 0) {
					cw1 = _realOwner.newMethod(Opcodes.ACC_STATIC | Opcodes.ACC_SYNTHETIC, _realOwner.accessorName(), "()"+type.toDesc());
					cw1.visitSize(type.length(), 0);
					cw1.field(Opcodes.GETSTATIC, owner, fn.name(), fn.rawDesc());
				} else {
					cw1 = _realOwner.newMethod(Opcodes.ACC_FINAL | Opcodes.ACC_SYNTHETIC, _realOwner.accessorName(), "()"+type.toDesc());
	...
	public void writeWrite(CodeWriter cw, String owner, FieldNode fn) {
		if (exist && cw.method.owner().equals(this.owner.name())) {
			...
			if (writeAccessor < 0) {
				...
				CodeWriter cw1;
				if ((fn.modifier & Opcodes.ACC_STATIC) != 0) {
					cw1 = _realOwner.newMethod(Opcodes.ACC_STATIC | Opcodes.ACC_SYNTHETIC, _realOwner.accessorName(), "("+type.toDesc()+")V");
					cw1.visitSize(type.length(), type.length());
					cw1.varLoad(type, 0);
					cw1.field(Opcodes.PUTSTATIC, owner, fn.name(), fn.rawDesc());
				} else {
					cw1 = _realOwner.newMethod(Opcodes.ACC_FINAL | Opcodes.ACC_SYNTHETIC, _realOwner.accessorName(), "("+type.toDesc()+")V");
