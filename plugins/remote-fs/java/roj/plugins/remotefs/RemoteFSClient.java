...
import roj.plugins.remotefs.proto.ClientPacket;
 ...
 */
@FastVarHandle
public class RemoteFSClient implements ChannelHandler, Consumer<MyChannel> {
	private static final Logger LOGGER = Logger.getLogger();
	private static final MSSContext context = new MSSContext().setALPN("RFS");
	private static final ChannelHandler PACKET_SERIALIZER = Packets.SERIALIZER.client();

	private String accessToken;
	public RemoteFSClient(String accessToken) {this.accessToken = accessToken;}

	public Source getRemoteSource(String path, boolean writable) throws IOException {
		var rpc = new RPCTask();
		rpc.type = 0;
		rpc.path = path;
		rpc.handle = writable ? Packets.OpenHandle.OPEN_WRITE : Packets.OpenHandle.OPEN_READ;
		executeSync(rpc);
	...
	private static final VarHandle STATE = Telescope.lookup().findVarHandle(RPCTask.class, "state", long.class);
	...
	public void channelTick(ChannelCtx ctx) throws Exception {
		if ((rpc == null) && !rpcTasks.isEmpty()) {
			...
			if (rpc != null) {
				...
				switch (rpc.type) {
					case 0 -> ctx.channelWrite(new Packets.OpenHandle(rpc.path, rpc.handle)); // handle
					case 1 -> ctx.channelWrite(new Packets.FileHandleOp(rpc.handle, Packets.FileHandleOp.META_READ, rpc.p0.writableBytes())); // readcount
					case 2 -> ctx.channelWrite(new Packets.FileData(rpc.handle, buf -> buf.put(rpc.p0.slice(Math.min(rpc.p0.readableBytes(), MAX_FILE_DATA))))); // writecount
					case 3 -> ctx.channelWrite(new Packets.FileHandleOp(rpc.handle, Packets.FileHandleOp.META_POSITION, rpc.p1));
					case 4 -> ctx.channelWrite(new Packets.FileHandleOp(rpc.handle, Packets.FileHandleOp.META_LENGTH, rpc.p1));
				}
			}
		}
	}
	...
	public void onFail(String message) throws IOException {
		...
		if (message.isEmpty()) {
			if (rpc.type <= 1) rpc.done("illegal response for type="+rpc.type);
				...
				ctx.channelWrite(new Packets.FileData(rpc.handle, buf -> buf.put(rpc.p0.slice(Math.min(rpc.p0.readableBytes(), MAX_FILE_DATA)))));
				return;
            }
		} else {
	...
	public void handleFileMeta(Packets.FileMetadata packet) {

	}

	public void handleDirectoryContent(Packets.DirectoryMeta packet) {

	}

	public void onOpenHandle(Packets.FileHandle packet) {
		if (rpc == null) return;
		if (rpc.type != 0) {
	...
	public void onFileData(Packets.FileData packet) throws IOException {
		...
		if (rpc.type != 1 || rpc.handle != packet.handle) {
			...
			if (preDone) {
				...
				ctx.channelWrite(new Packets.FileHandleOp(rpc.handle, Packets.FileHandleOp.CANCEL_FILEDATA, 0L));
			}
	...
	public void onLogin(String brand) throws IOException {
		...
		else ctx.channelWrite(new Packets.AccessToken(accessToken));
	...
	public class RemoteSource extends Source implements Consumer<Exception> {
		...
		@Override
		public Source copy() throws IOException {
			return null;
		}
