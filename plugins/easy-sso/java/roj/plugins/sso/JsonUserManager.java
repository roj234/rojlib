...
class JsonUserManager implements UserManager {
	...
	public JsonUserManager(File json) throws IOException, ParseException {
		this.json = json;
		this.users = !json.isFile() ? new ArrayList<>() : SERIALIZER.listOf(User.class).read(json, ConfigMaster.JSON);
	...
	private void doSave() {
		...
		try {
			SERIALIZER.listWriter(User.class).write(ConfigMaster.JSON, copy, json);
		} catch (Exception e) {
			e.printStackTrace();
			dirty = true;
		}
