...
public class EasySSO extends Plugin {
	private static final XashMap.Template<String, Group> TEMPLATE = XashMap.forType(String.class, Group.class).key("name").build();
	...
	private final XashMap<String, Group> groups = TEMPLATE.create();
	...
	public String login(Request req, String user, String pass, boolean is_token) throws IllegalRequestException {
		...
		if (is_token) {
			...
			if (u != null) {
				getLogger().info("用户{}在{}刷新令牌登录成功", user, req.remoteAddress());
		...
		if (u != null) {
			...
			if (pass.equals(u.tempOtp) || o.hasher.compare(u.passHash, IOUtil.encodeUTF8(pass))) {
				getLogger().info("用户{}在{}密码登录成功", user, req.remoteAddress());
			...
			if (u.totpKey != null) {
				...
				if (totp.equals(pass)) {
					getLogger().info("用户{}在{}验证码登录成功", user, req.remoteAddress());
					return loginSuccess(o, u, req, false);
				}
			}

			users.setDirty(u, "loginAttempt");
			getLogger().info("用户{}在{}登录失败", user, req.remoteAddress());
		}
	...
	public String register(Request req, String user, String pass) throws IllegalRequestException {
		...
		u.registerAddr = req.remoteAddress();
	...
	public String qrcode(Request req) {
		...
		qrLoginTemp.put(id, new QRLogin(id, "Unknown("+req.remoteAddress()+")的"+req.get("user-agent")));

		return "{\"ok\":true,\"url\":\""+(req.isSecure()?"https":"http")+"://"+req.host()+"/"+sitePath+"qrcode/"+id+"/apply\",\"id\":\""+id+"\",\"timeout\":"+ QRCODE_TTL +"}";
	}
			...
			if (System.currentTimeMillis() >= obj.expire) {
				...
				if (user != null) {
					...
					if (confirm != null) {
						...
						getLogger().info("用户{}在{}扫码登录{}成功", user, req.remoteAddress(), obj.address);
